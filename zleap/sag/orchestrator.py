...
SAGRuntime 负责客户端、存储端口、提示词和 chunk hydration；SAGSearcher
负责召回、扩展、排序、证据追踪与结果组装。可选 candidate scope 只限定
请求内候选域，不改变其余 SAG 执行链路。
"""
...
from .expand import SAGExpandStage
from .recall import SAGRecallStage
from .rerank import SAGRerankStage
from .routes import SAGRouteTracker
from .runtime import SAGRuntime
from .timing import SAGTimingService

logger = get_logger("search.sag")


class SAGSearcher:
    """SAG search entrypoint with infrastructure provided by SAGRuntime."""

    def __init__(
        self,
        runtime: SAGRuntime | None = None,
        *,
        rerank_client=None,
        settings=None,
    ):
        self._runtime = runtime or SAGRuntime()
        self._rerank_client = rerank_client
        self._settings = settings
        self._timing_service = SAGTimingService(
            self._runtime,
            ctx_var_name=f"sag_step7_llm_timing_{id(self)}",
        )
        self._recall_stage: SAGRecallStage = SAGRecallStage(
            runtime=self._runtime,
            timing=self._timing_service,
        )
        self._expand_stage: SAGExpandStage = SAGExpandStage(
            runtime=self._runtime,
            timing=self._timing_service,
        )
        self._rerank_stage: SAGRerankStage = SAGRerankStage(
            runtime=self._runtime,
            timing=self._timing_service,
    ...
    async def aclose(self) -> None:
        """Release resources owned by this searcher's SAG runtime."""

        await self._runtime.aclose()

    # ------------------------------------------------------------------
    # SAG-owned dependency accessors
    # ------------------------------------------------------------------
    ...
    async def search(
        ...
        *,
        ...
        Args:
            ...
            config: SAGConfig 配置（读取 sag_recall/sag_expand/sag_rerank）。
        ...
        Returns:
            {
                ...
                # + SAG 诊断字段（route_stats/event_scores/rank_method/...）
                # + evidence_coverage（gold_evidences 传入时）
            }
        """
        config = config or SAGConfig()
        if not source_config_ids:
            raise ValueError("SAG search 必须传入 source_config_ids")

        total_start = time.perf_counter()
        self._runtime.get_prompts(config)
        # 按 config 开关一次性加载提示词（委托 SAGRuntime 的实例级缓存）

        tracker = EvidenceTracker(set(gold_evidences or []))

        state = SAGSearchState()
        timings: dict[str, float] = dict.fromkeys(SAG_TIMING_STAGE_ORDER, 0.0)
        retry_wasted_by_stage: dict[str, float] = dict.fromkeys(SAG_TIMING_STAGE_ORDER, 0.0)
        ...
        # SAGSearcher instance.
        timing_context_token = self._timing_service.start_request()

        try:
            logger.info(
                "[SAG] 搜索开始: query=%r, rerank_strategy=%s, rewrite=%s, ner=%s, fast=%s",
                query,
                config.sag_rerank.strategy,
                config.sag_rewrite_query_enabled,
                config.sag_enable_entity_extraction,
                config.sag_use_fast_mode,
            )

            route_index = SAGRouteTracker.new_route_index(query)
            ...
            score_top_event_ids = top_event_ids_by_score(event_scores, limit=100)
            ...
            explicit_total = sum(
                timings[stage] for stage in SAG_TIMING_STAGE_ORDER if stage != "other_overhead"
            )
            ...
            logger.info(
                "[SAG] 搜索完成 total=%.2fs, items=%d, rank_method=%s",
            ...
            logger.error("[SAG] 搜索失败: %s", exc, exc_info=True)
            raise
        finally:
    ...
    async def _build_answer_clues_from_routes(
        ...
        needed_entity_ids = entity_ids_from_route_edges(selected_edges)
        needed_event_ids = event_ids_from_route_edges(selected_edges)
        ...
        stats = {
            "answer_events": len(merge_ids(answer_event_ids)),
        ...
        logger.info(
            "[SAG] 答案图谱构建完成: answer_events=%d, route_edges=%d, new_clues=%d",
            stats["answer_events"],
            stats["route_edges"],
            stats["total_clues"],
        )
    ...
    def _route_node_to_full_node(
        ...
        if node_type == "event":
            node = full_event_node(node_id, event_map.get(node_id, {}))
            node["hop"] = hop
            node["stage"] = "recall" if hop == 0 else "expand"
            return node
        node = full_entity_node(node_id, entity_map.get(node_id, {}))
        node["hop"] = hop
        return node
