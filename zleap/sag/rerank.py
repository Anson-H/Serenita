"""SAG ranking and fallback stage."""
...
from .contracts import SAGRequest, SAGRerankOutput, SAGRerankResult
...
logger = get_logger("search.sag.rerank")


@dataclass(slots=True)
class SAGRerankStage:
    runtime: SAGRuntime
    ...
    async def _load_rank_candidate_events(
        ...
        logger.info("[SAG] 排序候选详情: input=%d, loaded=%d", len(event_ids), len(ordered))
    ...
    async def rerank_events(
        ...
        rerank_top_k = min(
            config.sag_rerank.rerank_top_k,
            config.sag_rerank.max_results,
            len(documents),
        )
        try:
            ...
            rerank_results = await asyncio.wait_for(
                ...
                timeout=config.sag_rerank.rerank_timeout,
            )
        except Exception as exc:
            logger.warning("[SAG] rerank 失败/超时，降级为相似度排序: %s", exc)
            fallback_ids = ordered_event_ids[: config.sag_rerank.max_results]
            return (
                build_ranked_event_results(
        ...
        seen: set[str] = set()
        threshold = config.sag_rerank.rerank_score_threshold
        ...
        if rerank_selected_count < config.sag_rerank.max_results:
            ...
            for event_id in similarity_ordered_ids:
                if event_id in seen:
                    continue
                if event_scores.get(event_id, 0.0) < config.sag_rerank.score_threshold:
                    continue
                seen.add(event_id)
                rerank_event_ids.append(event_id)
                if len(rerank_event_ids) >= config.sag_rerank.max_results:
                    break
        ...
        logger.info(
            "[SAG] rerank 完成: input=%d, rerank_top_k=%d, rerank_selected=%d, "
    ...
    async def _llm_rank_events(
        ...
        """LLM rerank using the SAG-local prompt and schema.

        返回值末尾的 display_ids 是"LLM 真正选中"的子集，不含相似度补齐部分。
        """
        event_ids = merge_ids(event_ids)[: config.sag_rerank.llm_rank_top_n]
        if not event_ids:
            return [], [], {}, False, {}, [], None, 0
        ...
        for index, event_id in enumerate(ordered_event_ids):
            ...
            if config.sag_rerank.llm_rank_include_content:
                text = str(event.get("content") or "").strip()
                if text:
                    max_len = config.sag_rerank.llm_rank_max_content_len
                    text = text[:max_len]
                else:
        ...
        top_k = min(config.sag_rerank.llm_rank_max_results, len(ordered_event_ids))
        llm_client = None
        try:
            messages = self.runtime.get_prompts(config).get_sag_rerank_messages(
                question=query,
                relations=relations_str,
                top_k=top_k,
            )
            llm_client = await self.runtime.get_llm_client()
            parsed: SAGRerankOutput = await llm_client.chat_parsed(messages, SAGRerankOutput)
            ...
            logger.warning("[SAG] LLM rank 失败，降级为相似度排序: %s", exc)
            return (
                build_ranked_event_results(
        ...
        for line in parsed.useful_relations:
            ...
            if len(llm_event_ids) >= config.sag_rerank.max_results:
                break
        ...
        if not llm_event_ids:
            logger.warning("[SAG] LLM rank 未解析出有效事件，降级为相似度排序")
            return (
                build_ranked_event_results(
                    fallback_ids, event_map, event_scores, {}, "llm_rank_score"
                ),
        ...
        logger.info(
            "[SAG] LLM rank 完成: input=%d, output=%d, llm_selected=%d",
    ...
    async def _rrf_rank_events(
        ...
        # BM25 remains part of SAG RRF when event details come from scope.
        ...
        logger.info(
            "[SAG] RRF rank 完成: input=%d, bm25_hit=%d, output=%d",
            len(ordered_event_ids),
            len(bm25_scores),
            len(rrf_sorted),
        )
