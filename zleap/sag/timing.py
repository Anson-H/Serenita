"""Request-local LLM and stage timing service for SAG."""
...
from zleap.sag.contracts import SAG_TIMING_STAGE_ORDER
from zleap.sag.utils import round_seconds


class SAGTimingService:
    ...
    def build_timing_payload(
        ...
        for stage in SAG_TIMING_STAGE_ORDER:
        ...
        payload: dict[str, Any] = {
            "schema_version": 2,
            "stage_order": list(SAG_TIMING_STAGE_ORDER),
    ...
    def validate_timing_payload(payload: dict[str, Any]) -> None:
        ...
        if abs(with_total - payload["total_with_retry"]) > tolerance:
            raise ValueError("SAG timing v2 with-retry stages do not conserve total")
        if abs(no_total - payload["total_no_retry"]) > tolerance:
            raise ValueError("SAG timing v2 no-retry stages do not conserve total")
        if (
            abs(
            ...
            raise ValueError("SAG timing v2 retry waste does not conserve totals")
