"""SAG recall algorithm and stage orchestration."""
...
from .candidate_scope import SAGCandidatePoolBuilder, SAGCandidateSubgraph
...
from .routes import SAGRouteTracker
from .runtime import SAGRuntime
from .timing import SAGTimingService
from .utils import (
...
logger = get_logger("search.sag.recall")


@dataclass(slots=True)
class SAGRecallStage:
    runtime: SAGRuntime
    timing: SAGTimingService

    async def run(
        self,
        request: SAGRequest,
        state: SAGSearchState,
        ...
        if rewritten_query:
            SAGRouteTracker.add_rewritten_query(route_index, rewritten_query)
        ...
        t0 = time.perf_counter()
        scope = None
        if config.sag_scope.enabled:
            scope = await SAGCandidatePoolBuilder(
                self.runtime.search_events_by_vector,
                self.runtime.event_universe_store,
            ).build(
                query_vector=query_vector,
                source_config_ids=source_config_ids,
                event_top_k=config.sag_scope.event_top_k,
                bootstrap_entity_limit=config.sag_scope.bootstrap_entity_limit,
                include_event_content=config.sag_scope.include_event_content,
            )
        if scope is not None:
            self.timing.record_timed_stage(timings, retry_wasted_by_stage, "candidate_pool", t0)
            logger.info("[SAG] candidate scope enabled: %s", scope.stats())
        ...
        logger.info(
            "[SAG] events merged: query=%d, entity=%d, merged=%d, score_range=%s",
            len(query_event_ids),
        ...
        initial_route_stats = SAGRouteTracker.record_initial(
    ...
    async def _retrieve_entity_candidates(
        ...
        *,
        ...
        """SAG wrapper around the runtime entity retrieval dependency."""
        ...
        logger.info(
            "[SAG] entity vector recall: query_entities=%d, entities=%d",
            len(query_entities),
            len(result[0]),
        )
        return result
    ...
    async def _recall_query_to_event_in_scope(
        self, scope: SAGCandidateSubgraph, config: SAGConfig
    ) -> tuple[list[dict[str, Any]], dict[str, float]]:
        threshold = config.sag_recall.score_threshold
        score_cache = {
            event_id: score for event_id, score in scope.event_scores.items() if score >= threshold
        }
        events = [
            event
            for event in scope.top_events(config.sag_recall.query_recall_event_max)
    ...
    async def _recall_entity_event_candidates_in_scope(
        ...
        config: SAGConfig,
        state: SAGSearchState,
        scope: SAGCandidateSubgraph,
    ) -> tuple[dict[str, list[str]], list[str], list[str]]:
        if not scope.entity_ids:
            return {}, [], []
        if config.sag_enable_entity_extraction:
            ...
            query_entity_ids, _names, _scores = (
                await self._retrieve_entity_candidates(
                    ...
                    source_config_ids=source_config_ids,
                    entity_top_k=config.sag_recall.max_entities,
                    key_similarity_threshold=config.sag_recall.entity_vector_threshold,
                    state=state,
            ...
            entities = await self.runtime.search_entities_by_text(
                query=query,
                source_config_ids=source_config_ids,
                size=config.sag_recall.max_entities,
                allowed_entity_ids=list(scope.entity_ids),
            )
        ...
        entity_events, event_ids = scope.events_for_entities(
            query_entity_ids, limit_per_entity=config.sag_recall.max_events_per_key
        )
    ...
    async def _recall_query_to_event(
        ...
        """
        k = config.sag_recall.query_recall_event_max
        threshold = config.sag_recall.score_threshold
        ...
        logger.info(
            "[SAG] 路A query->event: raw=%d, kept=%d, cache=%d",
            len(raw_events),
            len(results),
            len(score_cache),
        )
    ...
    async def _recall_entity_event_candidates(
        ...
        实体入口：默认 BM25；若 sag_enable_entity_extraction 则用 SAG 的 step1/step2
        ...
        state.entity_ids.update(query_entity_ids)
        logger.info("[SAG] 路B 实体召回: entities=%d", len(query_entity_ids))
    ...
    async def events_from_entities(
        ...
        *,
        ...
        per_key = max_events_per_key or config.sag_recall.max_events_per_key
        ...
        logger.info(
            "[SAG] entity->event (MySQL): entities=%d, events=%d",
    ...
    async def _filter_entity_events_by_similarity(
        ...
        *,
        ...
        logger.info(
            "[SAG] entity events 相似度过滤: before=%d, cached=%d, after=%d",
            len(event_ids),
            len(cached_ids),
    ...
    async def _rewrite_query_and_extract_entities(
        ...
        query: str,
        config: SAGConfig,
    ) -> tuple[str | None, list[SAGEntity]]:
        """问题重写 + 实体提取。关闭/失败时返回 (None, [])。"""
        if not config.sag_rewrite_query_enabled:
            return None, []

        llm_client = None
        try:
            current_ts = local_now().isoformat(timespec="seconds")
            messages = self.runtime.get_prompts(config).get_sag_rewrite_messages(
                query=query,
                current_timestamp=current_ts,
                max_entities=config.sag_recall.max_entities,
            )
            llm_client = await self.runtime.get_llm_client()
            parsed: SAGRewriteOutput = await llm_client.chat_parsed(messages, SAGRewriteOutput)
            self.timing.record_llm_call_metrics()
        except Exception as exc:
            if getattr(llm_client, "_last_call_timing", None):
                self.timing.record_llm_call_metrics()
            logger.warning("[SAG] query 重写失败，回退原始 query: %s", exc)
            return None, []

        rewritten_query = (parsed.rewritten_query or "").strip()
        if not rewritten_query:
            logger.warning("[SAG] query 重写结果为空，回退原始 query")
            return None, []
        logger.info(
            "[SAG] query 重写完成: original=%r, rewritten=%r, entities=%d",
            query,
            rewritten_query,
            len(parsed.entities),
        )
