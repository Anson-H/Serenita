...
from .candidate_scope import SAGCandidateSubgraph
...
logger = get_logger("search.sag.expand")


@dataclass(slots=True)
class SAGExpandStage:
    runtime: SAGRuntime
    ...
    async def run(
        ...
        if not request.config.sag_expand.enabled or request.config.sag_expand.max_hops <= 0:
            return SAGExpandResult(
                event_ids=event_ids,
        ...
        route_stats = SAGRouteTracker.record_relation(
            route_index=route_index,
            pairs_mapping=seed_new_entity_events,
            event_scores=event_scores,
            method="seed_event_to_key",
            relation="SAG event->key",
            hop=1,
            from_hop=0,
            to_hop=1,
            key_is_event=False,
    ...
    async def _new_entities_from_seed_events_in_scope(
        ...
        seed_ids = merge_ids(
            ...
            for item in ranked[: config.sag_expand.seed_event_limit]
        )
        # Keep SAG's original event_entity_vectors similarity filter, but
        ...
        relation_map, relation_entities = await self._new_entities_from_event_ids(
            ...
            max_entities=config.sag_expand.entities_per_hop,
        ...
        for entity_id in relation_entities:
            ...
            if len(scoped_entities) >= config.sag_expand.entities_per_hop:
                break
    ...
    async def _expand_from_scope(
        ...
        for hop_no in range(1, config.sag_expand.max_hops + 1):
            if not current_entities:
                break
            candidate_map, candidate_ids = scope.events_for_entities(
                current_entities, limit_per_entity=config.sag_expand.max_events_per_hop
            )
            ...
            candidate_ids = sorted(
                ...
                reverse=True,
            )[: config.sag_expand.max_events_per_hop]
            hop_scores = {eid: scope.event_scores[eid] for eid in candidate_ids}
            threshold = config.sag_expand.event_similarity_threshold
    ...
    async def _new_entities_from_seed_events(
        ...
        seed_limit = config.sag_expand.seed_event_limit
        ...
        logger.info(
            "[SAG] seed events -> new entities: seeds=%d, new_entities=%d",
            len(seed_event_ids),
            len(new_entity_ids),
        )
    ...
    async def _new_entities_from_event_ids(
        ...
        relation_k = max_entities * config.sag_expand.relation_k_multiplier
        threshold = config.sag_expand.entity_relation_score_threshold

        logger.info(
            "[SAG] event -> new key 关系向量召回: events=%d, relation_k=%d, threshold=%.4f",
            len(event_ids),
            relation_k,
            threshold,
        )
        ...
        logger.info(
            "[SAG] event -> new key 关系向量召回完成: raw=%d, kept=%d, entities=%d, top=%s",
            raw_count,
            kept_count,
            len(new_entity_ids),
            new_entity_ids[:5],
    ...
    async def _events_from_new_entities(
        ...
        logger.info(
            "[SAG] new entity -> new event: candidates=%d, kept=%d",
    ...
    async def _expand_from_entities(
        ...
        hop_count = config.sag_expand.max_hops
        for hop in range(hop_count):
            if not current_entity_ids:
                logger.info("[SAG] expand 第 %d 跳无新实体，停止", hop + 1)
                break
            ...
            (
                ...
                config=config,
                state=state,
                max_events=config.sag_expand.max_events_per_hop,
            )
            ...
            logger.info(
                "[SAG] expand 第 %d 跳完成: events=%d, new_keys=%d",
                hop_no,
                len(hop_event_ids),
