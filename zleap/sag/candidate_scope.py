"""Bounded SAG candidate universe.
...
the event/entity edges. All subsequent SAG graph operations use the in-memory
...
class SAGCandidatePoolBuilder:
    """Build a request-scoped SAG subgraph with one vector and one graph-read phase."""
    ...
    async def build(
        ...
        *,
        ...
        if not scores:
            return SAGCandidateSubgraph(source_config_ids=tuple(source_config_ids))
