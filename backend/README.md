...
`create_app(paths=...)` 可显式指定数据根；命令行工厂优先使用 `DATA_ROOT`，未设置时自部署使用项目下的 `serenita_self_hosted_data/`，官方模式使用 `serenita_online_data/`。数据目录、账号配置、密钥与 Schema 生命周期见[本地存储](../docs/当前执行/运维/本地存储.md)。

默认采用自部署模式，每个实例只有一个本地账号，前端打开即进入同一工作区。空数据目录首次创建工作区身份，已有唯一账号直接使用；多账号数据目录禁止以自部署模式启动。一个工作区可管理多位家庭成员的资料。官方网站设置 `SERENITA_SERVICE_MODE=official`，支持多账号注册与登录，健康客户端和服务端管理页分别建立会话。自部署连接官方模型的地址由 `SERENITA_OFFICIAL_URL` 指定，完整部署参数见[官方模型服务](../docs/当前执行/运维/本地存储.md#官方模型服务)。
服务端管理 API 通过 `server.backend.main:create_app` 独立启动，本机端口为 8002；在线健康 API 仍在 8001。两个进程共同使用 `serenita_online_data/`，分别注册管理和健康接口，管理进程不启动健康业务调度。
...
`app/` 根目录包含 `main.py` 应用入口和包声明。`main.create_app()` 创建应用，由 `application/services.py` 的 `ApplicationServices` 按同一数据根装配 Service、Repository 和后台任务。官方模式、独立管理员初始化和自部署地址配置见[官方模型服务部署](../docs/当前执行/运维/本地存储.md#官方模型服务)。
...
```bash
uv run python -m backend.app.application.body_metrics.demo
```
