...
from backend.app.providers.errors import ProviderChatCompletionError, ProviderModelListError, request_failure_message
from backend.app.providers.capability_probe import (
    ...
    _PROBE_REPLY_PROMPT, _PROBE_THINKING_MAX_TOKENS,
)
...
def attempt(checks, errors, key, action):
    try:
        ...
        errors[key] = f"{label}：{request_failure_message(exc)}"
        return None
...
def detect_model_type(provider, *, api_url, api_key, remote_model_id, cancellation_token):
    ...
    def generation():
        output = provider.complete_chat(
            **args,
            model_request=ModelRequest.build(
                system="",
                messages=[{"role": "user", "content": _PROBE_REPLY_PROMPT}],
                model_config={"max_tokens": _PROBE_THINKING_MAX_TOKENS},
            ),
            timeout_seconds=provider.attachment_timeout_seconds,
        )
    ...
    try:
        ...
        metadata = {"status": "unavailable", "message": request_failure_message(exc)}
    if conflict:
