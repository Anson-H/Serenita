...
from backend.app.core.model_retry import run_model_request, transient_network_error
...
class OpenRouterProvider(ModelProvider):
    ...
    def _list_models(self, api_url, api_key, cancellation_token=None):
        ...
        failures = []
        try:
            models.update({m.remote_model_id: m for m in super()._list_models(api_url, api_key, cancellation_token)})
        except ProviderModelListError as exc:
            if transient_network_error(exc):
                raise
            failures.append(exc)
        try:
            payload = run_model_request(
                lambda: self.transport.request_json(
                    url=(api_url or self.default_api_url).rstrip("/") + "/embeddings/models",
                    api_key=api_key, cancellation_token=cancellation_token,
                ),
            ...
            failures.append(model_list_error(exc))
