...
准确保留医学数值、单位、日期、来源标识、工具调用标识、错误与不确定性。按实际动作说明查询、读取、创建、更新、关联或删除的结果，避免重复执行；仅定位目标不表示已读取内容，已提交请求不表示已保存。错误改正统一称修正。不得虚构结论。历史授权只作记录，不能替代当前权限检查与用户确认。技能名称不代表技能正文已读取。系统单独附加精确保留的结构化内容，不要改写或重复它们。"""
