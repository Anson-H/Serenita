...
from backend.app.core.business_operation import operation_scope, tool_operation_id
...
class AgentHarnessRuntime:
    ...
    def execute(
        ...
        *,
        ...
        events: list[AgentEvent] = []
        context.memory['successful_model_resource_refs'] = []
        ...
        while action_count < self.max_actions or pending_pages:
            ...
            from backend.app.agent_runtime.model_resource_reads import retain_successful_model_resources
            retain_successful_model_resources(context, request)
    ...
    def _execute_call(
        ...
        *,
        ...
        visible_arguments = dict(call.arguments) if isinstance(call.arguments, dict) else call.arguments
        emit(
            AgentEvent(
                type="tool_call",
                payload={
        ...
        if isinstance(call.arguments, str):
            try:
                ...
                if isinstance(exc, json.JSONDecodeError):
                    message = f"工具参数不是有效 JSON：{exc.msg}。"
                    details = {"line": exc.lineno, "column": exc.colno, "position": exc.pos}
                invalid_key = f"{call.name}:schema_validation"
                repeated_calls[invalid_key] = repeated_calls.get(invalid_key, 0) + 1
                code = "TOOL_ARGUMENTS_INVALID"
                if repeated_calls[invalid_key] > self.max_repeated_calls:
                    suspended_tools.add(call.name)
                    code = "REPEATED_INVALID_TOOL_ARGUMENTS"
                    message += " 同一工具已连续提交无效结构，当前轮次已暂停该工具。"
                return self._emit_tool_error(
                    call, code=code,
                    message=message + " 此次调用未执行，请根据错误和工具参数 Schema 决定下一步。",
                    details=details, emit=emit,
                )
            call = replace(call, arguments=parsed_arguments)
            finish_arguments["call"] = call
        ...
        try:
            ...
            with operation_scope(tool_operation_id(context.account_id, context.session_id,
                                                   context.turn_id, call.id), "agent_tool"), tool_cancellation_scope(context.cancellation_token):
    ...
    def _default_request_token_estimate(request: ModelRequest) -> int:
        return estimate_input_tokens(request.canonical_dict())
    ...
    def _model_runtime_context(context: AgentContext) -> dict[str, Any]:
        ...
        return {
            ...
            **({"execution_budget": dict(context.memory["execution_budget"])}
               if context.memory.get("execution_budget") is not None else {}),
            **(
