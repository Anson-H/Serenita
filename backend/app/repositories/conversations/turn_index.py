from backend.app.storage.conversations.database import CONVERSATION_DATABASE_SCHEMA
...
class TurnIndex:
    ...
    def insert_turn(
        ...
        if stream_id != f"stream_{turn_id}":
            raise ValueError("流标识必须由轮次标识生成。")
        ...
        with connect(self.paths.conversations_db(account_id)) as connection:
            cursor = connection.execute(
                ...
                INSERT INTO conversation_turns (
                    ...
                    final_assistant_message_id, status, error_code, error_message,
            ...
            existing = connection.execute(
                """
                SELECT user_message_id, final_assistant_message_id, 'stream_' || turn_id AS stream_id, created_at
    ...
    def expire_turn_jobs(self, account_id: str, session_id: str, cutoff: str) -> list[dict[str, Any]]:
        ...
        with connect(self.paths.conversations_db(account_id)) as connection:
            rows = connection.execute(
                ...
                RETURNING *, 'stream_' || turn_id AS stream_id
                """,
    ...
    def update_turn_completed(
        ...
        *,
        ...
        from backend.app.repositories.conversations.notification_outbox import ConversationNotificationOutboxRepository

        outbox = ConversationNotificationOutboxRepository(self.paths)
        with connect(self.paths.conversations_db(account_id)) as connection:
            ...
            if notification_event and updated.rowcount == 1:
                outbox.enqueue(connection, **notification_event)
    ...
    def turn_row(self, account_id: str, session_id: str, turn_id: str):
        with connect(self.paths.conversations_db(account_id)) as connection:
            return connection.execute(
                """
                SELECT *, 'stream_' || turn_id AS stream_id FROM conversation_turns
                WHERE session_id = ? AND turn_id = ?
                """,
                (session_id, turn_id),
            ).fetchone()
    ...
    def turn_by_stream_id(self, account_id: str, session_id: str, stream_id: str):
        ...
        with connect(self.paths.conversations_db(account_id)) as connection:
            return connection.execute(
                """
                SELECT *, 'stream_' || turn_id AS stream_id FROM conversation_turns
                WHERE session_id = ? AND ('stream_' || turn_id) = ?
                """,
