...
from backend.app.repositories.conversations.index import ConversationIndex
from backend.app.repositories.conversations.turn_index import TurnIndex
from backend.app.repositories.conversations.attachments import ConversationAttachments
import time
...
from backend.app.core.runtime.member_lifecycle import member_lifecycle_operation, member_lifecycle_change
...
from backend.app.storage.conversations.session_persistence import (
...
class ConversationRepository:
    ...
    def session_event_summary(self, account_id: str, session_id: str) -> tuple[int, bool]:
        if self.session_row(account_id, session_id) is None:
            raise_error("missing", "NOT_FOUND", "聊天不存在。")
        try:
            return self.persistence.summary(account_id, session_id)
    ...
    def append_session_events(
        ...
        *,
        ...
        for attempt in range(128):
            try:
                seq = self.persistence.next_sequence(account_id, session_id)
            except UnsupportedSessionLogError as exc:
                raise UnsupportedSchemaError(
                    "UNSUPPORTED_SCHEMA: session event log is not current."
                ) from exc
