...
from backend.app.repositories.reports.changes import report_write
from backend.app.storage.sqlite import connect
from backend.app.repositories.reports.transaction import ReportTransaction
from backend.app.repositories.reports.values import _row_dict
...
class ReportSources:
        ...
        try:
            with self.transaction(write=True) as transaction:
                ...
                if self.active_business_command is not None:
                    self.active_business_command.new_files.add((member_id, relative_path))
                ...
                source = dict(resource_id=resource_id, member_id=member_id,
                    ...
                    sha256=signature, created_at=local_now_iso())
                db.execute(
                    """INSERT INTO report_sources
                    (resource_id, member_id, mime_type, size_bytes, relative_path, sha256, created_at)
                    VALUES (:resource_id, :member_id, :mime_type, :size_bytes, :relative_path, :sha256, :created_at)""",
                    source,
                )
    ...
    def drain_file_cleanup(self, *, member_id: str | None = None, limit: int = 8) -> None:
        """Remove a bounded batch of committed, unreferenced source files.

        Original mutations authorize and enqueue removals. Cleanup also runs after
        member deletion and therefore does not require a current member identity.
        The file-removal operation checks live references under its storage lock.
        Failed jobs remain in the durable outbox for a subsequent attempt.
        """
        try:
            jobs = self.file_cleanup_jobs(member_id, limit=limit)
        except Exception:
            return
        drain_cleanup_batch(jobs, self.clean_source_file,
            lambda job: self.record_file_cleanup_attempt(job["member_id"], job["cleanup_id"]))
    ...
    def record_file_cleanup_attempt(self, member_id: str, cleanup_id: str) -> bool:
        ...
        with self.connection() as connection:
            cursor = connection.execute(
                ...
                """,
                (local_now_iso(), member_id, cleanup_id),
            )
