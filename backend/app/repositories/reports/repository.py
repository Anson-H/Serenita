...
from backend.app.repositories.reports.changes import ReportChanges, report_write, report_snapshot, source_snapshot
...
from backend.app.repositories.reports.sources import ReportSources
from backend.app.repositories.reports.queries import ReportQueries
from contextlib import contextmanager
from backend.app.repositories.reports.lab_catalog import LabCatalog
from backend.app.repositories.reports.facts import ReportFacts
from backend.app.repositories.reports.transaction import ReportTransaction
from backend.app.repositories.reports.values import _lab_value_key
from backend.app.storage.report_database import REPORT_DATABASE_SCHEMA
from backend.app.domain.report_errors import ReportImportWriteConflictError
...
class ReportRepository(ReportChanges, ReportSources, ReportQueries):
    """SQLite persistence for an owner's member-scoped reports and source files."""

    def __init__(self, account_id: str, paths: Optional[AppPaths] = None, *, actor_account_id: str | None = None):
        self._account_id = account_id
        self.actor_account_id = actor_account_id or account_id
        self.paths = paths or app_paths()
        self.facts = ReportFacts(self.account_id, self.paths, watch_report=self.watch_report_on_connection, reserve_report_id=self.reserve_report_id)
    ...
    def delete_member_on_connection(
        ...
        *,
        ...
        if replay is not None:
            return
        for row in connection.execute(f"SELECT report_id FROM {schema}.reports WHERE member_id=?", (member_id,)).fetchall():
            before = report_snapshot(connection, member_id, row["report_id"], schema_alias=schema_alias)
            record_change(connection, actor_account_id=self.account_id, operation_id=operation.operation_id,
                          scope_kind="member", member_id=member_id, resource_type="report", resource_id=row["report_id"],
                          before=before, after=None, context={"member_id": member_id, "report_name": before["/report_name"], "report_time": before["/report_time"]},
                          origin_kind=operation.origin_kind, schema_alias=schema_alias)
        for row in connection.execute(f"SELECT resource_id FROM {schema}.report_sources WHERE member_id=?", (member_id,)).fetchall():
            record_change(connection, actor_account_id=self.account_id, operation_id=operation.operation_id,
                          scope_kind="member", member_id=member_id, resource_type="report_source", resource_id=row["resource_id"],
                          before=source_snapshot(connection, member_id, row["resource_id"], schema_alias=schema_alias), after=None,
        ...
        finish_operation(connection, self.account_id, operation.operation_id,
    ...
    def save_report_analysis(
        ...
        with self.connection() as connection:
            ...
            cursor = connection.execute(
                ...
                WHERE member_id = ? AND report_id = ?
                  AND (analysis_content <> ? OR analysis_outdated <> 0)
                """,
                (
                    analysis_content,
                    timestamp,
            ...
            if cursor.rowcount != 1 and connection.execute("SELECT 1 FROM reports WHERE member_id=? AND report_id=?", (member_id, report_id)).fetchone() is None:
                return None
        return self.get_report(member_id, report_id)
    ...
    def _replace_structured_data_on_connection(
        ...
        *,
        ...
        before = report_snapshot(connection, member_id, report_id)
        previous_audit = connection.execute("SELECT analysis_outdated,updated_at FROM reports WHERE member_id=? AND report_id=?", (member_id, report_id)).fetchone()
        ...
        after = report_snapshot(connection, member_id, report_id)
        if before is not None and after is not None:
            evidence_before = {key: value for key, value in before.items() if key != "/analysis_outdated"}
            evidence_after = {key: value for key, value in after.items() if key != "/analysis_outdated"}
            if evidence_before == evidence_after:
                connection.execute("UPDATE reports SET analysis_outdated=?,updated_at=? WHERE member_id=? AND report_id=?",
                                   (previous_audit["analysis_outdated"], previous_audit["updated_at"], member_id, report_id))
