...
from backend.app.domain.report_errors import LabDictionaryMergeResultConflictError
...
class ReportFacts:
    ...
    def __init__(self, account_id, paths, *, watch_report=None, reserve_report_id=None):
        self.account_id = account_id
        self.paths = paths
        self.watch_report = watch_report or (lambda *_: None)
        self.reserve_report_id = reserve_report_id or (lambda *_: None)
    ...
    def enqueue_file_cleanup(
        ...
        *,
        ...
        connection.execute(
            ...
            """,
            (cleanup_id, member_id, relative_path, local_now_iso()),
        )
        return True
    ...
    def next_report_id(
        ...
        base = f"{prefix}-{day}-"
        rows = connection.execute(
            "SELECT report_id FROM reports WHERE report_id LIKE ? "
            "UNION SELECT resource_id AS report_id FROM business_changes "
            "WHERE resource_type = 'report' AND resource_id LIKE ?", (f"{base}%", f"{base}%")
        ).fetchall()
        maximum = 0
        for row in rows:
            try:
                ...
                continue
        report_id = f"{base}{maximum + 1:04d}"
        self.reserve_report_id(report_id)
        return report_id
    ...
    def copy_report_sources(
        ...
        self.watch_report(db, member_id, target_report_id)
        rows = db.execute(
