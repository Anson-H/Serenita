...
from backend.app.storage.models.codec import capability_column_values
...
class ModelProviderTransaction:
    ...
    def get_provider(self, *, provider_id):
        ...
        return self.connection.execute(
            "SELECT *, encrypted_api_key IS NOT NULL AS is_configured FROM model_providers WHERE provider_id = ?", values
        ).fetchone()
    ...
    def get_model(self, *, model_id):
        values = (model_id,)
        return self.connection.execute(
            "SELECT models.*, model_providers.provider_name FROM models JOIN model_providers USING (provider_id) WHERE model_id = ?", values
        ).fetchone()
    ...
    def default_model(self, purpose):
        ...
        return self.connection.execute(
            f"""
        SELECT models.*, model_providers.provider_name
        FROM model_access_settings
        JOIN models ON models.model_id = model_access_settings.{column_name}
        JOIN model_providers ON model_providers.provider_id = models.provider_id
    ...
    def rename_provider(self, *, provider_id, provider_name):
        self.validate_write("model_providers", {"provider_name": provider_name}, previous=self.get_provider(provider_id=provider_id))
        self.connection.execute("UPDATE model_providers SET provider_name=? WHERE provider_id=?", (provider_name, provider_id))
    ...
    def update_provider(
        ...
        *,
        ...
        self.validate_write('model_providers', dict(zip(('api_url','official_url','encrypted_api_key','updated_at','provider_id'), values)), self.get_provider(provider_id=provider_id))
        return self.connection.execute(
            """
    ...
    def save_capability_detection(self, model_id, report):
        import json
        value = json.dumps(report, ensure_ascii=False)
        self.validate_write('models', {'capability_detection': value}, self.get_model(model_id=model_id))
        self.connection.execute("UPDATE models SET capability_detection = ? WHERE model_id = ?", (value, model_id))

    def list_models(self):
        values = ()
        return self.connection.execute(
            "SELECT models.*, model_providers.provider_name FROM models JOIN model_providers USING (provider_id) ORDER BY models.created_at", values
        ).fetchall()

    def list_providers(self):
        return self.connection.execute("SELECT * FROM model_providers ORDER BY created_at, provider_id").fetchall()

    def delete_provider(self, provider_id):
        self.connection.execute("DELETE FROM models WHERE provider_id = ?", (provider_id,))
        self.connection.execute("DELETE FROM model_providers WHERE provider_id = ?", (provider_id,))
