...
from backend.app.repositories.members.references import member_reference_stores
from backend.app.repositories.members.lifecycle_repository import MemberLifecycleRepository

from backend.app.core.business_operation import current_business_operation, bind_business_operation
...
from backend.app.core.runtime.member_lifecycle import member_lifecycle_change, member_lifecycle_guard
from backend.app.repositories.members.data_store import (
...
class MemberRepository:
    def __init__(
        ...
        *,
        ...
        self.authorization = MemberAuthorizationRepository(self.paths)
    ...
    def member_exists(self, member_id: str) -> bool:
        with self.authorization.read() as connection:
            return (
                connection.execute(
    ...
    def create_on_connection(
        ...
        *,
        ...
        command = {"domain": "member", "action": "create", "values": values, "set_as_default": set_as_default}
        member_id = str(uuid.uuid5(uuid.NAMESPACE_URL, json.dumps(["serenita:member", account_id, operation.operation_id], separators=(",", ":"))))
        ...
        previous_result = begin_operation(connection, account_id, operation.operation_id, command, schema_alias=member_schema)
        if previous_result is not None:
            return previous_result["member_id"]
        connection.execute(
        ...
        connection.execute(
            """INSERT INTO member_data.members
                (member_id, member_name, relationship, sex, birth_date, blood_type, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                member_id,
                values["member_name"],
        ...
        current = connection.execute(f'SELECT * FROM "{member_schema}".members WHERE member_id = ?', (member_id,)).fetchone()
        record_change(connection, actor_account_id=account_id, operation_id=operation.operation_id,
                      scope_kind="member", member_id=member_id, resource_type="member", resource_id=member_id,
                      before=None, after=self._profile_business_values(current),
                      context={"member_id": member_id, "member_name": current["member_name"]},
                      origin_kind=operation.origin_kind, schema_alias=member_schema)
        finish_operation(connection, account_id, operation.operation_id, {"member_id": member_id}, schema_alias=member_schema)
        return member_id
    ...
    @staticmethod
    def _profile_business_values(record):
        return {f"/{field}": record[field] for field in ("member_name", "relationship", "sex", "birth_date", "blood_type")}
    ...
    def update(
        ...
        *,
        ...
        if set(values) - {"member_name", "relationship", "sex", "birth_date", "blood_type"}:
            member_error(
                "MEMBER_FIELDS_INVALID", "包含不可更新的成员字段。", "invalid_input"
            )
        operation = current_business_operation()
        command = {"domain": "member", "action": "update", "member_id": access.member_id,
                   "values": values, "set_as_default": set_as_default}
        with connect(self.paths.auth_db) as connection:
            ...
            previous_result = begin_operation(connection, live.actor_account_id, operation.operation_id, command, schema_alias="member_data")
            if previous_result is not None:
                return
            previous = connection.execute("SELECT * FROM member_data.members WHERE member_id = ?", (live.member_id,)).fetchone()
            if previous is None:
                member_error("MEMBER_NOT_FOUND", "成员不存在。", "missing")
            changed = {field: value for field, value in values.items() if previous[field] != value}
            if changed:
                assignments = ", ".join(f"{field} = ?" for field in changed)
                updated = connection.execute(
                    f"UPDATE member_data.members SET {assignments}, updated_at = ? WHERE member_id = ?",
                    (*changed.values(), local_now_iso(), live.member_id),
                )
                if not updated.rowcount:
                    member_error("MEMBER_NOT_FOUND", "成员不存在。", "missing")
            if set_as_default:
            ...
            current = connection.execute("SELECT * FROM member_data.members WHERE member_id = ?", (live.member_id,)).fetchone()
            record_change(connection, actor_account_id=live.actor_account_id, operation_id=operation.operation_id,
                          scope_kind="member", member_id=live.member_id, resource_type="member", resource_id=live.member_id,
                          before=self._profile_business_values(previous), after=self._profile_business_values(current),
                          context={"member_id": live.member_id, "member_name": current["member_name"]},
                          origin_kind=operation.origin_kind, schema_alias="member_data")
            finish_operation(connection, live.actor_account_id, operation.operation_id, {"member_id": live.member_id}, schema_alias="member_data")
    ...
    def save_preferences(
        ...
        *,
        ...
        actor_id = actor_account_id
        operation = current_business_operation()
        values = {key: value for key, value in (("startup_mode", startup_mode),
                  ("default_member_id", default_member_id), ("last_member_id", last_member_id))
                  if value is not _PREFERENCE_UNSET and (key != "startup_mode" or value is not None)}
        with connect(self.paths.auth_db) as connection:
            config_schema = self._attach_config_database(
                connection,
                actor_id,
            ...
            replay = begin_operation(connection, actor_id, operation.operation_id,
                                     {"domain": "member_preference", "action": "update", "values": values},
                                     schema_alias=config_schema)
            if replay is not None:
                return
            before = self._preference_on_connection(
                connection,
                schema_alias=config_schema,
            )
            ...
            current = self._preference_on_connection(connection, schema_alias=config_schema)
            fields = ("startup_mode", "default_member_id", "last_member_id")
            record_change(connection, actor_account_id=actor_id, operation_id=operation.operation_id,
                          scope_kind="account", member_id=None, resource_type="member_preference", resource_id="1",
                          before={f"/{key}": before[key] for key in fields}, after={f"/{key}": current[key] for key in fields},
                          context={"preference_kind": "member"}, origin_kind=operation.origin_kind, schema_alias=config_schema)
            finish_operation(connection, actor_id, operation.operation_id, {"saved": True}, schema_alias=config_schema)
    ...
    def delete(
        ...
        with connect(self.paths.auth_db) as connection:
            connection.execute("BEGIN EXCLUSIVE")
            self._attach_member_database(connection, actor_id, schema_alias="member_data")
            replay = begin_operation(connection, actor_id, operation.operation_id,
                                     {"command": "member.delete", "member_id": member_id},
                                     schema_alias="member_data")
            if replay is not None:
                return replay["account_id"], [tuple(item) for item in replay["sessions"]]
            access = self._access(connection, actor_id, member_id)
            ...
            for table, fields in (
                ("members", ("member_name", "relationship", "sex", "birth_date", "blood_type")),
                ("medical_history", MEDICAL_HISTORY_FIELDS),
            ):
                prior = connection.execute(f'SELECT * FROM member_data.{table} WHERE member_id=?',
                                           (member_id,)).fetchone()
                if prior is not None:
                    record_change(connection, actor_account_id=actor_id, operation_id=operation.operation_id,
                                  scope_kind="member", member_id=member_id,
                                  resource_type="member" if table == "members" else "medical_history",
                                  resource_id=member_id, before={f"/{name}": prior[name] for name in fields},
                                  after=None, context={"member_id": member_id}, origin_kind=operation.origin_kind,
                                  schema_alias="member_data")
            ...
            finish_operation(connection, actor_id, operation.operation_id,
                             {"account_id": access.account_id, "sessions": [list(item) for item in sessions]},
                             schema_alias="member_data")
    ...
    def revoke(
        ...
        with connect(self.paths.auth_db) as connection:
            ...
            previous_grant = connection.execute(
                "SELECT updated_at FROM member_grants WHERE member_id=? AND account_id=?",
            ...
            if not removed.rowcount:
                return []
            from backend.app.repositories.memory.sources.restrictions import restrict_revoked_grant

            restrict_revoked_grant(connection, self.paths, access.account_id, member_id,
                                  actor_id, account_id, str(previous_grant["updated_at"]))
            config_schema = self._attach_config_database(
                connection,
                account_id,
                schema_alias="grantee_config",
