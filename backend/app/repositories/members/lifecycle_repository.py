...
class MemberLifecycleRepository:
    def __init__(self, paths):
        self.path = paths.auth_db

    def initialize(self):
        if not AUTH_DATABASE_SCHEMA.validate_existing(self.path):
            with connect(self.path) as db:
                db.execute("BEGIN IMMEDIATE")
                AUTH_DATABASE_SCHEMA.create(db)
    ...
    def enqueue(self, connection, owner, member, sessions, *, delete_files=False):
        ...
        main = next(row for row in databases if row[1] == "main")
        if not main[2] or Path(main[2]).resolve() != self.path.resolve():
            raise UnsupportedSchemaError("成员后续任务必须使用文件认证主库的原事务。")
        ...
        connection.executemany(
            "INSERT INTO member_lifecycle_tasks "
