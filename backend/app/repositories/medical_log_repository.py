...
class MedicalLogRepository:
    def __init__(self, account_id, paths, *, actor_account_id=None):
    ...
    def transaction(self, *, write=False):
        ...
        with connect(self.paths.medical_logs_db(self.account_id)) as connection:
            ...
            if write:
                ...
                bind_deletion_journal(connection, self.paths, self.account_id, 'medical_logs.db')
            yield connection
    ...
    @staticmethod
    def _business_values(record):
        return {f"/{field}": record[field] for field in ("recorded_on", "title", "content")}

    @staticmethod
    def _change_context(member_id, record):
        return {"member_id": member_id, "recorded_on": record["recorded_on"], "title": record["title"]}

    def create(self, member_id, values):
        operation = current_business_operation()
        command = {"domain": "medical_log", "action": "create", "member_id": member_id, "values": values}
        with self.transaction(write=True) as connection:
            previous_result = begin_operation(connection, self.actor_account_id, operation.operation_id, command)
            if previous_result is not None:
                self._detail(connection, member_id, previous_result["medical_log_id"])
            ...
            result = self._detail(connection, member_id, medical_log_id)
            record_change(connection, actor_account_id=self.actor_account_id, operation_id=operation.operation_id,
                          scope_kind="member", member_id=member_id, resource_type="medical_log", resource_id=medical_log_id,
                          before=None, after=self._business_values(result), context=self._change_context(member_id, result), origin_kind=operation.origin_kind)
            return finish_operation(connection, self.actor_account_id, operation.operation_id, result)

    def update(self, member_id, medical_log_id, values):
        operation = current_business_operation()
        command = {"domain": "medical_log", "action": "update", "member_id": member_id, "medical_log_id": medical_log_id, "values": values}
        with self.transaction(write=True) as connection:
            previous_result = begin_operation(connection, self.actor_account_id, operation.operation_id, command)
            if previous_result is not None:
                self._detail(connection, member_id, previous_result["medical_log_id"])
            ...
            record_change(connection, actor_account_id=self.actor_account_id, operation_id=operation.operation_id,
                          scope_kind="member", member_id=member_id, resource_type="medical_log", resource_id=medical_log_id,
                          before=self._business_values(current), after=self._business_values(result), context=self._change_context(member_id, result), origin_kind=operation.origin_kind)
            return finish_operation(connection, self.actor_account_id, operation.operation_id, result)

    def delete(self, member_id, medical_log_id):
        operation = current_business_operation()
        command = {"domain": "medical_log", "action": "delete", "member_id": member_id, "medical_log_id": medical_log_id}
        with self.transaction(write=True) as connection:
            previous_result = begin_operation(connection, self.actor_account_id, operation.operation_id, command)
            if previous_result is not None:
                return previous_result
            ...
            record_change(connection, actor_account_id=self.actor_account_id, operation_id=operation.operation_id,
                          scope_kind="member", member_id=member_id, resource_type="medical_log", resource_id=medical_log_id,
                          before=self._business_values(current), after=None, context=self._change_context(member_id, current), origin_kind=operation.origin_kind)
            result = {"member_id": member_id, "medical_log_id": medical_log_id, "deleted": True}
            return finish_operation(connection, self.actor_account_id, operation.operation_id, result)
