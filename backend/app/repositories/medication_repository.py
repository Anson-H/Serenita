...
from backend.app.core.business_operation import current_business_operation, validate_operation_id
...
class MedicationRepository:
    ...
    def transaction(self, write=False):
        ...
        with connect() as db:
            ...
            if write:
                ...
                bind_deletion_journal(db, self.paths, self.account_id, 'medications.db', schema_alias='medication_data')
            yield db
    ...
    def detail(self, db, member, kind, object_id):
        ...
        result = decode(row)
        if kind == "plan":
            result["medication_identity"] = self.medication_identity(db, result["medication_id"])
        if kind == "plan":
        ...
        if kind == "medication":
            ...
            result["sources"] = [public_source(s) for s in MedicationSourceRepository.list(db, object_id)]
            result["batches"] = [
                decode(r)
                for r in db.execute(
        ...
        with self.transaction() as db:
            ...
            for item in items:
                ...
                if medication_fields is not None and 'sources' in medication_fields:
                    from backend.app.repositories.medication_source_repository import MedicationSourceRepository, public_source
                    item['sources'] = [public_source(source) for source in MedicationSourceRepository.list(db, item['medication_id'])]
                if kind == "plan":
    ...
    def medication_identity(self, db, medication_id):
    ...
    def persist(
        ...
        if creating:
            ...
            payload = {**encode(values), "updated_at": local_now_iso()}
    ...
    def save(self, actor, member, kind, changes, *, object_id=None, operation_id=None, medication_id=None, sources=()):
        ...
        command_id = validate_operation_id(operation_id) if operation_id is not None else operation.operation_id
        command = {"domain": "medication", "member_id": member, "action": "create" if object_id is None else "update",
                   "kind": kind, "object_id": object_id, "values": changes, "medication_id": medication_id,
                   "sources": source_command(sources)}
        source_repo = MedicationSourceRepository(self) if sources else None
        with (source_repo.staging() if source_repo else nullcontext([])) as created, self.transaction(True) as db:
            prior = begin_operation(db, actor, command_id, command, schema_alias="medication_data")
            if prior is not None:
                return prior
            current = self.detail(db, member, kind, object_id) if object_id else None
            if kind == "batch" and current is not None and medication_id is not None and current["medication_id"] != medication_id:
                raise LookupError("批次不属于该药品。")
            model = MODELS[kind]
            values = model.model_validate({
                **{key: value for key, value in (current or {}).items() if key in model.model_fields},
                **changes,
            }).model_dump()
            if kind == "plan":
                self.medication_identity(db, values["medication_id"])
            if kind == "batch":
                parent = medication_id if current is None else current["medication_id"]
                self.medication_identity(db, parent)
                values["medication_id"] = parent
            if current is None or any(dump(value) != dump(current.get(key)) for key, value in values.items()):
                object_id = self.persist(db, member, kind, values, object_id)
            if source_repo:
                source_repo.link(db, member, object_id, sources, created, reject_duplicates=True)
            result = self.detail(db, member, kind, object_id)
            before, after = current, result
            if kind == "batch":
                after = {**result, "medication_identity": self.medication_identity(db, result["medication_id"])}
                before = {**current, "medication_identity": self.medication_identity(db, current["medication_id"])} if current is not None else None
            self.record_object(db, actor, command_id, operation.origin_kind, member, kind, object_id, before, after)
            if source_repo:
                self.record_sources(db, actor, command_id, operation.origin_kind, object_id, [], result["sources"], result)
            return finish_operation(db, actor, command_id, result, schema_alias="medication_data")

    def add_sources(self, actor, member, medication_id, sources, *, operation_id):
        from backend.app.repositories.medication_source_repository import MedicationSourceRepository
        operation = current_business_operation()
        command_id = validate_operation_id(operation_id)
        command = {"domain": "medication", "action": "add_sources", "member_id": member,
                   "medication_id": medication_id, "sources": source_command(sources)}
        source_repo = MedicationSourceRepository(self)
        with source_repo.staging() as created, self.transaction(True) as db:
            prior = begin_operation(db, actor, command_id, command, schema_alias="medication_data")
            if prior is not None:
                return prior
            previous = self.detail(db, member, "medication", medication_id)
            source_repo.link(db, member, medication_id, sources, created, reject_duplicates=True)
            result = self.detail(db, member, "medication", medication_id)
            self.record_sources(db, actor, command_id, operation.origin_kind, medication_id,
                                previous["sources"], result["sources"], result)
            return finish_operation(db, actor, command_id, result, schema_alias="medication_data")

    @staticmethod
    def record_object(db, actor, operation_id, origin_kind, member, kind, object_id, before, after):
        return record_change(
            db, actor_account_id=actor, operation_id=operation_id,
            scope_kind="account" if kind == "medication" else "member",
            member_id=None if kind == "medication" else member,
            resource_type={"medication": "medication", "plan": "medication_plan", "batch": "medication_batch"}[kind],
            resource_id=object_id, before=medication_fields(kind, before), after=medication_fields(kind, after),
            context=medication_context(kind, before, after), origin_kind=origin_kind, schema_alias="medication_data",
        )

    @staticmethod
    def record_sources(db, actor, operation_id, origin_kind, medication_id, before, after, medication):
        old, new = ({row["resource_id"]: row for row in rows} for rows in (before, after))
        for resource_id in sorted(old.keys() | new.keys()):
            record_change(
                db, actor_account_id=actor, operation_id=operation_id,
                scope_kind="account", member_id=None, resource_type="medication_source", resource_id=resource_id,
                before=source_fields(old.get(resource_id)), after=source_fields(new.get(resource_id)),
                context={"medication_id": medication_id,
                         "medication_identity": {key: medication[key] for key in Identity.model_fields}},
                origin_kind=origin_kind, schema_alias="medication_data",
            )

    def delete(self, actor, member, kind, object_id, *, medication_id=None):
        operation = current_business_operation()
        with self.transaction(True) as db:
            prior = begin_operation(db, actor, operation.operation_id, {
                "domain": "medication", "action": "delete", "kind": kind,
                "member_id": member, "object_id": object_id, "medication_id": medication_id,
            }, schema_alias="medication_data")
            if prior is not None:
                return prior
            previous = self.detail(db, member, kind, object_id)
            if kind == "batch" and medication_id is not None and previous["medication_id"] != medication_id:
                raise LookupError("批次不属于该药品。")
            if kind == "batch":
                previous["medication_identity"] = self.medication_identity(db, previous["medication_id"])
            if kind == 'medication':
            ...
            self.record_object(db, actor, operation.operation_id, operation.origin_kind, member, kind, object_id, previous, None)
            if kind == "medication":
                self.record_sources(db, actor, operation.operation_id, operation.origin_kind,
                                    object_id, previous["sources"], [], previous)
            result = {"deleted": True, IDS[kind]: object_id, **({"member_id": member} if member is not None else {})}
            return finish_operation(db, actor, operation.operation_id, result, schema_alias="medication_data")
...
def source_command(sources):
    return [{key: source[key] for key in ("original_filename", "mime_type", "sha256", "purpose")}
...
def medication_fields(kind, row):
    if row is None:
        return None
    fields = {}
    for name in (*MODELS[kind].model_fields, *(("medication_id",) if kind == "batch" else ())):
        value = row[name]
        if name != "schedule" or value is None:
            fields[f"/{name}"] = value
            continue
        for key, content in value.items():
            path = f"/schedule/{key}"
            if key == "times" and content:
                for item in content:
                    fields[f"{path}/{_pointer(item['time'])}/time"] = item["time"]
            elif key == "weekdays" and content:
                for weekday in content:
                    fields[f"{path}/{weekday}"] = weekday
            else:
                fields[path] = content
    return fields
...
def source_fields(row):
    return None if row is None else {
        f"/{key}": row[key] for key in ("medication_id", "original_filename", "mime_type", "size_bytes",
                                       "sha256", "source_index", "purpose", "is_primary")
    }


def medication_context(kind, before, after):
    row = after if after is not None else before
    if row is None:
        return {"kind": kind}
    identity = {key: row[key] for key in Identity.model_fields} if kind == "medication" else row.get("medication_identity")
    return {"kind": kind, "medication_id": row.get("medication_id"), "medication_identity": identity}


def snapshot_medication_member(db, member_id, schema_alias="medication_data"):
    """Read actual member-owned plans and batches before a same-transaction delete."""
    import re
    if not isinstance(schema_alias, str) or not re.fullmatch(r"[a-z][a-z0-9_]*", schema_alias):
        raise ValueError("数据库别名无效。")
    result = []
    for kind in ("plan", "batch"):
        rows = db.execute(
            f"SELECT * FROM {schema_alias}.{TABLES[kind]} WHERE member_id=?", (member_id,),
        ).fetchall()
        for raw in rows:
            row = decode(raw)
            identity = db.execute(
                f"SELECT {','.join(Identity.model_fields)} FROM {schema_alias}.medications WHERE medication_id=?",
                (row["medication_id"],),
            ).fetchone()
            row["medication_identity"] = dict(identity) if identity is not None else None
            result.append({"resource_type": "medication_plan" if kind == "plan" else "medication_batch",
                           "resource_id": row[IDS[kind]], "before": medication_fields(kind, row),
                           "context": medication_context(kind, row, None)})
    return result
