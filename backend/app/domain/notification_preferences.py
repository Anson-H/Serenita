...
def reception_since(preferences, notification_type):
    if not preferences[f"{notification_type}_enabled"]:
        return None
    return max(
...
def effective_preferences(row):
    result = dict(row)
    result["notifications_enabled"] = any(bool(result[f"{name}_enabled"]) for name in NOTIFICATION_TYPES)
    return result
