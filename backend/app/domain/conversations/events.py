...
_REQUIRED_DATA_FIELDS: dict[str, frozenset[str]] = {
    ...
    "model/retry": frozenset(
        {"turn_id", "step", "call_id", "first_call_id", "retry", "created_at"}
    ),
...
def _validate_event_data(event_type: str, data: Mapping[str, Any]) -> None:
    if event_type in {"model/retry", "model/result"} and "retry" in data:
        _validate_retry_metadata(data, event_type)
    if event_type == "model/retry":
        for field in ("turn_id", "call_id", "created_at"):
            if not isinstance(data[field], str) or not data[field].strip():
                raise SessionEventCorruptionError(f"model/retry {field} must be non-blank text")
        try:
            datetime.fromisoformat(data["created_at"])
        except ValueError as exc:
            raise SessionEventCorruptionError("model/retry created_at must be an ISO datetime") from exc
        if data.get("error") is not None and not isinstance(data["error"], dict):
            raise SessionEventCorruptionError("model/retry error must be an object")
    ...
    if event_type == "request/header":
        ...
        if "retry" in header:
            _validate_retry_metadata(header, "request/header.header")
...
def _validate_retry_metadata(data: Mapping[str, Any], label: str) -> None:
    retry = data.get("retry")
    fields = {"attempt", "max_attempts", "delay_seconds", "status"}
    if not isinstance(retry, dict) or set(retry) != fields:
        raise SessionEventCorruptionError(f"{label} retry must contain {sorted(fields)}")
    if not isinstance(data.get("first_call_id"), str) or not data["first_call_id"].strip():
        raise SessionEventCorruptionError(f"{label} first_call_id must be non-blank text")
    if type(retry["attempt"]) is not int or not 1 <= retry["attempt"] <= 10:
        raise SessionEventCorruptionError(f"{label} retry.attempt must be between 1 and 10")
    if type(retry["max_attempts"]) is not int or retry["max_attempts"] != 10:
        raise SessionEventCorruptionError(f"{label} retry.max_attempts must be 10")
    if retry["status"] not in ("waiting", "running", "completed", "failed", "cancelled"):
        raise SessionEventCorruptionError(f"{label} retry.status is invalid")
    expected_delay = 5 if retry["status"] == "waiting" else 0
    if type(retry["delay_seconds"]) is not int or retry["delay_seconds"] != expected_delay:
        raise SessionEventCorruptionError(f"{label} retry.delay_seconds must be {expected_delay}")
    if retry["status"] == "waiting" and retry["attempt"] == 10:
        raise SessionEventCorruptionError(f"{label} retry cannot wait after the last attempt")
