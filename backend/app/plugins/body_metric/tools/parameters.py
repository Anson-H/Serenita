...
FIELD_DOCS = {
    ...
    "starts_at": "发生或开始日期时间，必填本地 ISO 8601 日期时间字符串；不可用上传时间代替进食时间。",
    "ends_at": "结束日期时间，采用本地时间且晚于开始；区间、日汇总、睡眠和运动必填，其它可为 null。",
    "timezone": "系统本地时区元数据；省略由系统填写，显式提供必须与系统时区一致。",
        ...
        "create_body_record": schema(
            {
                "record": full,
                "operation_id": string(
                    "当前创建操作的稳定操作标识，须为 1 至 200 个不含空白的可打印 ASCII 字符；重试复用，同标识不同内容拒绝。"
                ),
            },
            ["record", "operation_id"],
        ),
