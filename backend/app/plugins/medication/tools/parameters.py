    ...
    return object_parameters(
        {
            **schema["properties"],
            "operation_id": {**string(
                "本次创建的稳定操作标识，1 至 200 个不含空白的可打印 ASCII 字符；同一命令重试复用，相同标识不同内容失败。"
        ...
        schema.get("required", []) + ["operation_id"] + (["medication_id"] if kind == "batch" else []),
    )
...
def medication_sources_parameters():
    return object_parameters({
        ...
        'operation_id': {**string('本次补充原件的稳定操作标识，1 至 200 个不含空白的可打印 ASCII 字符；同一命令重试复用，相同标识不同内容失败。'), 'maxLength': 200, 'pattern': '^[!-~]+$'},
    }, ['medication_id', 'sources', 'operation_id'])
