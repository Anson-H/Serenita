...
class MedicationTool(Tool):
    ...
    def _run(self, arguments):
        ...
        if op == "catalog":
            ...
            output = self.service.add_sources(actor, member, v["medication_id"], uploads, v["operation_id"])
        elif op == "create":
            operation_id = v.pop("operation_id")
            ...
            output = self.service.save(
                actor, member, kind, v, operation_id=operation_id, medication_id=parent, uploads=uploads
            )
        elif op == "update":
