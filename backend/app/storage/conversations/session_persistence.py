...
from backend.app.storage.conversations.session_recovery import interrupted_turn_closers
...
class SessionPersistence(ABC):
    ...
    @abstractmethod
    def next_sequence(self, account_id: str, session_id: str) -> int:
        """Read the append position without returning the session contents."""
    ...
    @abstractmethod
    def summary(self, account_id: str, session_id: str) -> tuple[int, bool]:
        """Read queue count and fork availability without copying event bodies."""
        raise NotImplementedError
...
class JsonlSessionPersistence(SessionPersistence):
    ...
    def __init__(self, *, paths=None):
        ...
        self._summaries: OrderedDict[Path, tuple[str, tuple[int, bool]]] = OrderedDict()

    def summary(self, account_id: str, session_id: str) -> tuple[int, bool]:
        path = self._path(account_id, session_id)
        with _artifact_lock(self._lock_path(account_id, session_id)):
            revision = self.revision(account_id, session_id)
            with self._snapshot_guard:
                cached = self._summaries.get(path)
                if cached and cached[0] == revision:
                    self._summaries.move_to_end(path)
                    return cached[1]
            stored = self._read_locked(path, repair_torn=False)
            if stored is None:
                raise SessionEventCorruptionError("session event artifact is missing")
            ...
            for event in stored.events:
                if event.type == "input/queued":
                    input_id = str(event.data.get("input_id") or "")
                    if input_id:
                        queued.add(input_id)
                elif event.type == "input/queue-removed":
                    queued.discard(str(event.data.get("input_id") or ""))
                elif event.type == "user/message":
                    queued.discard(str(event.data.get("queued_input_id") or ""))
                elif event.type == "turn/end":
                    fork_available = True
            result = (len(queued), fork_available)
            with self._snapshot_guard:
                self._summaries[path] = (stored.revision, result)
                self._summaries.move_to_end(path)
                while len(self._summaries) > 256:
    ...
    def delete(self, account_id: str, session_id: str) -> None:
        ...
        try:
            ...
            # The original is already gone. Failure to remove its empty lock
            # file is cleanup work, not an unfinished source deletion.
            logging.getLogger(__name__).warning(
                'Conversation lock cleanup failed after source deletion: session=%s error=%s',
