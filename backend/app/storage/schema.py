...
class Column:
    ...
    def value_checks(self) -> tuple[CheckConstraint, ...]:
        expressions = []
        if self.storage_type in {"TEXT", "DATE", "DATETIME", "TIME"} and self.requires_content:
...
class Index:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.columns and not self.expressions:
            raise ValueError(f"index {self.name} requires columns or expressions")
        if any(not expression.strip() or ';' in expression for expression in self.expressions):
            raise ValueError(f"index {self.name} has an invalid expression")
    ...
    def sql(self, table_name: str) -> str:
        ...
        statement = (
            ...
            f"ON {table_name} ({', '.join((*self.columns, *self.expressions))})"
        )
        if self.where:
...
class Table:
    ...
    def __post_init__(self) -> None:
        ...
        self._require_known_columns(self.mutable_columns, "mutable column")
        if self.mutable_columns and (not self.append_only or set(self.mutable_columns) & set(self.primary_key)):
            raise ValueError('Mutable exceptions require protected rows and cannot include primary keys')
    ...
    def create_table_sql(self) -> str:
        ...
        body = ",\n    ".join(clauses)
        suffix = " WITHOUT ROWID" if self.append_only else ""
        return f"CREATE TABLE IF NOT EXISTS {self.name} (\n    {body}\n){suffix}"

    @property
    def value_checks(self) -> tuple[CheckConstraint, ...]:
        checks = [(position, check) for position, column in enumerate(self.columns)
                  for check in column.value_checks()]
        checks.extend((_check_column_order(check.expression, self.column_names), check) for check in self.checks)
        # Stable sorting keeps a column's generated checks before its business
        # rules. Cross-column rules belong to the earliest referenced column.
        return tuple(check for _, check in sorted(checks, key=lambda item: item[0]))
    ...
    def create_trigger_sql(self) -> tuple[str, ...]:
        """Protect committed rows, including replacement through a unique key."""
        if not self.append_only:
            return ()
        rejection = "SELECT RAISE(ABORT, 'APPEND_ONLY_VIOLATION');"
        update_condition = ''
        if self.mutable_columns:
            update_condition = ' WHEN ' + ' OR '.join(
                f'OLD.{name} IS NOT NEW.{name}' for name in self.column_names if name not in self.mutable_columns)
        statements = [
            f"CREATE TRIGGER IF NOT EXISTS {self.name}_deny_update "
            f"BEFORE UPDATE ON {self.name}{update_condition} BEGIN {rejection} END",
            f"CREATE TRIGGER IF NOT EXISTS {self.name}_deny_delete "
            f"BEFORE DELETE ON {self.name} BEGIN {rejection} END",
        ]
        keys = [(self.primary_key, None, ())]
        keys.extend((constraint.columns, None, ()) for constraint in self.unique_constraints)
        keys.extend((index.columns, index.where, index.expressions) for index in self.indexes if index.unique)
        collisions = []
        for columns, predicate, expressions in keys:
            incoming = ", ".join(f"NEW.{column.name} AS {column.name}" for column in self.columns)
            comparisons = [f"{name} = NEW.{name}" for name in columns]
            comparisons.extend(f"({expression}) = (SELECT ({expression}) FROM (SELECT {incoming}))" for expression in expressions)
            match = " AND ".join(comparisons)
            if predicate:
                incoming = ", ".join(f"NEW.{column.name} AS {column.name}" for column in self.columns)
                match += f" AND ({predicate}) AND (SELECT ({predicate}) FROM (SELECT {incoming}))"
            collisions.append(f"EXISTS (SELECT 1 FROM {self.name} WHERE {match})")
        # SQLite REPLACE can delete a conflicting row without firing DELETE
        # triggers when recursive_triggers is disabled on another connection.
        statements.append(
            f"CREATE TRIGGER IF NOT EXISTS {self.name}_deny_replace "
            f"BEFORE INSERT ON {self.name} WHEN {' OR '.join(collisions)} "
            f"BEGIN {rejection} END"
        )
...
class Database:
    ...
    def create(self, connection: sqlite3.Connection) -> None:
        ...
        for table in self.tables:
            ...
            for statement in table.create_trigger_sql():
                connection.execute(statement)
    ...
    def _validate_triggers(self, connection: sqlite3.Connection) -> None:
        ...
        if set(actual) != set(expected) or any(
            _canonical_sql(actual[name]) != _canonical_sql(statement)
            for name, statement in expected.items()
        ):
            raise UnsupportedSchemaError(
                f"UNSUPPORTED_SCHEMA: database {self.name} has unsupported triggers."
            )
...
def _check_column_order(expression: str, columns: tuple[str, ...]) -> int:
    """Let SQLite resolve references; literals and function names are not columns."""
    references = set()

    def record_reference(action, table, column, database, trigger):
        if action == sqlite3.SQLITE_READ and table == "check_columns":
            references.add(column)
        return sqlite3.SQLITE_OK

    with closing(sqlite3.connect(":memory:")) as connection:
        connection.execute(f"CREATE TABLE check_columns ({', '.join(columns)})")
        connection.set_authorizer(record_reference)
        connection.execute(f"SELECT ({expression}) FROM check_columns")
    return next((position for position, name in enumerate(columns) if name in references), len(columns))
...
def _canonical_sql(sql: str) -> str:
    # Quoted SQL values affect CHECK conditions and partial unique indexes.
    # Preserve them byte-for-byte while normalizing insignificant whitespace.
    parts = re.split(r"('(?:''|[^'])*'|\"(?:\"\"|[^\"])*\"|`(?:``|[^`])*`|\[(?:]]|[^]])*])", _strip_constraint_labels(sql))
    for position in range(0, len(parts), 2):
        parts[position] = re.sub(
            r"\s+", " ", re.sub(r"\bIF\s+NOT\s+EXISTS\b", "", parts[position], flags=re.IGNORECASE)
        )
