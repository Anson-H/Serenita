...
def object_table(kind):
    ...
    return Table(TABLES[kind], (
        ...
        *(col(n, storage={'starts_at': 'DATETIME', 'expires_on': 'DATE'}.get(n, 'TEXT'), nullable=type(None) in get_args(model.model_fields[n].annotation), non_blank=True if n in ('brand_name', 'ends_at', 'expires_on', 'leaflet_url') else None, json_kind='object' if n == 'schedule' else None) for n in data),
        *(col(n, G.STATE, False) for n in states), col('created_at', G.AUDIT, False, storage='DATETIME'), col('updated_at', G.AUDIT, False, storage='DATETIME'),
...
MEDICATION_DATABASE_SCHEMA = Database('medications.db', (
    ...
    Table('medication_sources', (
        ...
        col('created_at', G.AUDIT, False, storage='DATETIME'),
    ...
    Table('medication_source_cleanup_outbox', (
        col('cleanup_id', G.PRIMARY_KEY, False), col('relative_path', nullable=False), col('created_at', G.AUDIT, False, storage='DATETIME'),
    ), ('cleanup_id',)),
    # 用药计划。
    object_table('plan'),
