...
from backend.app.application.body_metrics.imports import CSV_FIELDS, MAX_UPLOAD
import csv, io, json
...
def statistics(
    ...
    return svc.statistics(user.account_id, member_id, after, before, category, source)
...
def create(
    member_id: str,
    payload: dict = Body(...),
    operation_id: str | None = Header(default=None, alias="X-Serenita-Operation-ID"),
    user=Depends(require_current_user),
    svc=Depends(service),
):
    return svc.create(user.account_id, member_id, payload, operation_id)
...
def template(
    ...
    from backend.app.application.body_metrics.demo import template_records
