...
from backend.app.application.reports.source_store import ReportSourceStore
from backend.app.application.reports.presenter import model_report_evidence, safe_detail
from backend.app.application.reports.thumbnail import render_source_thumbnail
from backend.app.application.reports.validation import (
...
from backend.app.repositories.reports.values import REPORT_PREFIXES
    ...
    def decorate(method):
        ...
        def authorized(self, member_id, *args, **kwargs):
            ...
            with self.scope.guard(write=write) as live:
                ...
                if write:
                    bound = inspect.signature(method).bind(self, member_id, *args, **kwargs)
                    bound.apply_defaults()
                    # Permission receipts and filesystem lookup metadata are runtime inputs.
                    values = {key: value for key, value in bound.arguments.items()
                              if key not in {"self", "visible_attachments", "authorized_report_sources"}}
                    result = self.repository.run_business_command(
                        {"domain": "medical_report", "action": method.__name__, "values": command_value(values)},
                        lambda: method(self, member_id, *args, **kwargs),
                        actor_account_id=live.actor_account_id, drain_files=True,
                    )
                else:
                    result = method(self, member_id, *args, **kwargs)
            if write and method.__name__ != "write_report_analysis":
                from backend.app.core.runtime.notification_runtime import notification_runtime
...
class ReportService:
    ...
    def __init__(
        ...
        *,
        ...
        self.repository = repository or ReportRepository(self.account_id, self.paths, actor_account_id=scope.actor_account_id)
        if (
    ...
    def read_report_sources(self, member_id: str, report_id: str) -> dict[str, Any]:
        ...
        for source_index, source in enumerate(detail.get("sources") or []):
            ...
            path = self.sources.source_path(source)
    ...
    def list_reports(
        ...
        *,
        ...
        self.repository.drain_file_cleanup()
    ...
    def add_report_sources(
        ...
        *,
        ...
        self.repository.drain_file_cleanup()
        if self.repository.get_report(member_id, report_id) is None:
            raise_error(
                "missing",
                "REPORT_NOT_FOUND",
