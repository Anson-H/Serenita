...
from backend.app.core.time import local_timezone
...
from backend.app.application.body_metrics.imports import parse_file
from backend.app.core.errors import raise_error
from backend.app.core.business_operation import operation_scope
...
class BodyMetricService:
    def __init__(self, members=None, *, read_only=False):
    ...
    def query(
        ...
        with self.repository(actor, member) as repo:
            filters = RecordFilter(after, before, category, source)
    ...
    def _statistics(self, repo, member, filters):
        ...
        result = {
            "series": stats(records, filters.after, filters.before),
    ...
    def statistics(
        ...
        with self.repository(actor, member) as repo:
            result = self._statistics(
                repo, member, RecordFilter(after, before, category, source)
            )
            return {
                **result,
                "series": [
    ...
    def series_data(
        ...
        *,
        ...
        with self.repository(actor, member) as repo:
            ...
            series = next(
                item
                for item in stats(records, after, before)
    ...
    def create(self, actor, member, values, operation_id=None):
        with self.repository(actor, member, write=True) as repo:
            return repo.create(member, values, operation_id)
    ...
    def start_import(self, actor, member, import_id, selection):
        with self.repository(actor, member, write=True) as repo:
            ...
            with self.lock:
                if key in self.jobs:
                    return repo.begin_import(member, import_id, selection), False
    ...
    def run_import(self, actor, member, import_id):
        execution_id = str(
            uuid5(NAMESPACE_URL, f"serenita:body-import:{actor}:{member}:{import_id}")
        )
        try:
            with (
                operation_scope(execution_id, "background_import"),
                self.repository(actor, member, write=True) as repo,
            ):
                repo.commit_import(member, import_id)
        except Exception as exc:
            try:
                failure_id = str(
                    uuid5(NAMESPACE_URL, f"{execution_id}:failure:{digest(str(exc))}")
                )
                with (
                    operation_scope(failure_id, "background_import"),
                    self.repository(actor, member, write=True) as repo,
                ):
