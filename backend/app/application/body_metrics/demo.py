...
def generate_records(today=None,days=90):
    ...
    def record(kind,at,key,data,end=None,metric='',precision=None):
        return {'kind':kind,'metric':metric,'starts_at':at.isoformat(),'ends_at':end.isoformat() if end else None,'precision':precision or ('interval' if end else 'instant'),'timezone':local_timezone_name(),'source':'虚构演示数据','device':'Serenita 演示设备','origin':'demo','external_id':f'demo:{key}:{at.isoformat()}','notes':'用于界面体验的虚构数据，不代表任何人的实际健康状况。','data':data}
