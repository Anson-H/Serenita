...
from backend.app.session_event_queries import current_session_turn_ids
        ...
        for event in events:
            ...
            if str(event.data.get("status") or "") != "completed":
                ...
                if details.get("execution_completed") is not True:
                    continue
            parse_call_id = str(event.data.get("call_id") or "")
            result = event.data.get("result")
            ...
            unresolved = [
                ...
                and (parse_call_id, int(item[index_field])) not in handled
            ]
            if unresolved:
