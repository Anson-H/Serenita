...
from backend.app.application.notifications.types import NotificationType
...
def reminder_delivery_allowed(row, plan):
    elapsed = local_now() - datetime.fromisoformat(row["occurred_at"])
...
def batch_expiration(batch):
    ...
    return (datetime.fromisoformat(batch["expires_on"]) + timedelta(days=1)).astimezone()
...
def build_types(paths):
    ...
    def present_expired(row, batch):
        ...
        if (expiration is None or expiration > local_now()
...
class MedicationNotificationProducer:
    ...
    def next_job(self, actor, member, plan, after):
        from backend.app.application.notifications.scheduler import ScheduledCall

        due = first_occurrence(plan, after, datetime(9998, 1, 1, tzinfo=local_timezone()))
        if due is None:
            return None
    ...
    def recover_actor(self, actor, at):
        ...
        recent = max(at, local_now()) - REMINDER_WINDOW
