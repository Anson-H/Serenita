...
class ReportSourceStore:
        ...
        *,
        ...
        if source_type == "conversation_text":
            ...
            if not raw_text.strip() or not message_id:
                raise ValueError("当前用户消息没有可绑定的报告文本。")
        ...
        if source_type == "report_source":
            ...
            if stored is None:
                raise PermissionError("引用的报告原件不属于当前账号或目标报告。")
            return (
                {
                    "source_type": source_type,
    ...
    def ensure_parsed_sources(
        ...
        *,
        ...
        if not parsed_sources:
            raise ValueError("每份解析报告必须绑定至少一个来源。")
        ...
        *,
        ...
        if source_type == "conversation_attachment":
            ...
            if not conversation_resource_id or not source_path.is_file():
                raise_error(
                    "missing", "REPORT_SOURCE_NOT_FOUND", "原始报告附件已经不可用。"
                )
            content = source_path.read_bytes()
            if not expected_sha or hashlib.sha256(content).hexdigest() != expected_sha:
                raise_error(
                    "conflict", "REPORT_SOURCE_CHANGED", "报告附件完整性校验失败。"
                )
            ...
            source = self.persist_normalized_upload(
        ...
        if (
            ...
            raise_error(
                "conflict", "REPORT_TEXT_SOURCE_CHANGED", "报告文本来源完整性校验失败。"
            )
        ...
        if existing is not None:
            ...
            return existing, False
        destination = self.paths.report_attachment_path(
            self.account_id, report_resource_id, "txt"
        )
        destination.parent.mkdir(parents=True, exist_ok=True)
        destination.write_bytes(content)
        destination.chmod(0o600)
        try:
            source, _created = self.repository.register_source_file(
                member_id,
                resource_id=report_resource_id,
                relative_path=str(
                    destination.relative_to(self.paths.account_root(self.account_id))
                ),
                mime_type="text/plain",
    ...
    def normalize_upload(self, upload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not filename or extension not in ALLOWED_EXTENSIONS:
            raise_error(
                ...
                "仅支持 JPG、PNG、HEIC 和 PDF 报告文件。",
            )
        if not isinstance(content, (bytes, bytearray)):
            raise_error("invalid_input", "INVALID_REQUEST", "上传文件内容无效。")
        if len(content) > MAX_FILE_BYTES:
            raise_error(
                "resource_limit", "FILE_TOO_LARGE", "单个报告文件不能超过 20MB。"
            )
        if not content:
    ...
    def persist_normalized_upload(
        ...
        *,
        ...
        extension = upload["extension"].lstrip(".")
        destination = self.paths.report_attachment_path(
            self.account_id, resource_id, extension
        )
        destination.parent.mkdir(parents=True, exist_ok=True)
        try:
        ...
        try:
            ...
            source, created = self.repository.register_source_file(
                member_id,
                resource_id=resource_id,
