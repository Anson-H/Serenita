...
from backend.app.application.conversations.streaming import (
    sse_event,
    stream_text_chunks,
)
        ...
        def events():
            ...
            while True:
                ...
                for record in projected_records:
                    ...
                    retry_changed = (
                        record.get("kind") == "model"
                        and record.get("channel") == "retry"
                        and sent.get("retry") != record.get("retry")
                    )
                    if retry_changed or (
                        record_terminal and not bool(sent.get("completed"))
                    ):
                        yield self.projected_record_completed_stream_event(
                            current_turn,
                            record,
                        )
                        sent["completed"] = record_terminal
                        if retry_changed:
                            sent["retry"] = dict(record["retry"])
                        emitted = True
