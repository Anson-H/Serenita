...
from backend.app.application.models.provider_service import (
    ModelProviderService,
)
from backend.app.domain.conversations.model_history import (
    derive_model_messages,
    tool_result_is_model_visible,
)
...
from backend.app.storage.conversations.session_persistence import SessionEventWriteConflictError
...
class ConversationCompaction:
    ...
    def compact_model_request_if_needed(
        ...
        *,
        ...
        try:
            ...
            def estimate_summary(request):
                return self.estimate_model_request_tokens(
                    ModelProviderService.prepare_transport_request(
                        compact_model,
                        request,
                        self.thinking_mode_for_model(compact_model, "default"),
                    ),
                    model=compact_model,
                )
    ...
    def estimate_model_request_tokens(
        model_request: ModelRequest, *, model=None
    ) -> int:
        return estimate_input_tokens(model_request.canonical_dict(), model=model)
