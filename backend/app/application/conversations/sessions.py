...
import re
from backend.app.core.runtime.member_lifecycle import (
...
class ConversationSessionCommands:
    def __init__(
        self, *, repository, paths, events, inputs, jobs, task_state, titles, queries
    ):
    ...
    def batch_pin_conversations(
        ...
        return {"sessions": [self.queries.summary(account_id, row) for row in rows]}
    ...
    def batch_delete_conversations(
        ...
        for session_id in unique_ids:
            try:
                self._cancel_for_deletion(account_id, session_id)
            except Exception as exc:
                preparation_errors[session_id] = exc
        for session_id in unique_ids:
            try:
                if session_id in preparation_errors:
                    raise preparation_errors[session_id]
                self._delete_cancelled_conversation(account_id, session_id)
    ...
    def _cancel_for_deletion(self, account_id, session_id):
        self.titles.cancel(account_id, session_id)
        with (
    ...
    def _delete_cancelled_conversation(self, account_id, session_id):
        # The exclusive boundary excludes event writes and queue scheduling.
        with (
            member_lifecycle_guard(paths=self.paths, exclusive=True),
            self.task_state.session_lock(account_id, session_id),
        ):
