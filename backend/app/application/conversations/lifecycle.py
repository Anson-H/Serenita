...
class ConversationTurnLifecycle:
    ...
    def interrupt_expired_turn_jobs(self, account_id: str) -> None:
        ...
        if not self.paths.conversations_db(account_id).is_file():
            return
        ...
        for session_id in self.repository.expired_turn_job_sessions(account_id, cutoff):
            with (
                ...
                for turn in self.repository.expire_turn_jobs(
    ...
    def execute_turn(
        ...
        *,
        ...
        try:
            harness_result = self.execution.run(
                account_id,
                turn,
                user_message,
            ...
            response_model_id = harness_result["model_id"]
            terminal = harness_result.get("terminal_action")
            ...
            if attempts and attempts > 1:
                error_message = f"{error_message}（已尝试 {attempts} 次）"
            persist_failed_output(
                getattr(exc, "code", None) or "MODEL_ERROR", error_message
            )
            return
    ...
    def _append_completed_turn_messages(
        ...
        self.repository.update_turn_completed(
            ...
            notification_event=completion_event(
                account_id, turn["session_id"], turn["turn_id"], timestamp
            ),
        )
