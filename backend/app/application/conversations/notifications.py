...
def build_types(paths):
    def load(scope, ids):
        rows = NotificationRepository(paths).completed_conversations(
            scope.actor_account_id, ids
        )
        result = {}
        for row in rows:
            entry = result.setdefault(
                row["session_id"], {"title": row["title"], "turns": set()}
            )
    ...
    return [
        NotificationType(
            "answer_completed", "conversation", load, present, requires_member=False
        )
    ]
