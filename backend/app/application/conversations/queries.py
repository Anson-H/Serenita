...
from backend.app.application.conversations.presenter import (
    record_response,
    message_response,
)
...
class ConversationQueries:
    def __init__(self, *, repository, members, events, plugin_service):
    ...
    def current_records(self, account_id: str, session_id: str) -> list[dict[str, Any]]:
    ...
    def summary(self, account_id: str, row) -> dict[str, Any]:
        queued_input_count, fork_available = self.repository.session_event_summary(
            account_id, str(row["session_id"])
        )
        ...
        return {
            ...
            "queued_input_count": queued_input_count,
            "fork_available": member_accessible and fork_available,
        }
