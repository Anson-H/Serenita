    ...
    def prepare(
        ...
        *,
        ...
        for ref, path, name, mime_type in resolved:
            ...
            original = ref.get("content_bytes")
            if not isinstance(original, bytes) and path is not None:
                original = path.read_bytes()
            if isinstance(original, bytes):
                safe_ref["file_sha256"] = hashlib.sha256(original).hexdigest()
            ...
            if mime_type == "text/plain":
                ...
                safe_ref["rendered_sha256"] = hashlib.sha256(
            ...
            if (
                mime_type == "application/pdf"
                and not self.inputs.model_can_forward_attachment(selected, mime_type)
            ):
                if path is None and not isinstance(ref.get("content_bytes"), bytes):
                    raise ProviderChatCompletionError("模型 PDF 资源缺少可读取原件。")
            ...
            try:
                ...
                raise ProviderChatCompletionError("工具返回的原件无法读取。") from exc
            part_type = next(
                (
                    kind
