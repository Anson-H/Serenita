    ...
    *,
        ...
        return [
            {
                ...
                "mime_type": mime_type,
                "name": path.name,
                "data_base64": base64.b64encode(content_bytes if content_bytes is not None else path.read_bytes()).decode("ascii"),
            }
        ]
    if content_bytes is not None:
        with tempfile.TemporaryDirectory(prefix="serenita-render-input-") as directory:
            ...
            return model_file_parts(source, mime_type, deadline=deadline, max_pages=max_pages)
