"""将图片或 PDF 转换为模型可接收的内容片段，限制渲染页数与时间。"""
...
def model_file_parts(
    ...
    *,
        ...
        return [
            {
                ...
                "data_base64": base64.b64encode(
                    content_bytes if content_bytes is not None else path.read_bytes()
                ).decode("ascii"),
            }
        ]
    if content_bytes is not None:
        with tempfile.TemporaryDirectory(prefix="serenita-render-input-") as directory:
            source = Path(directory) / "source.pdf"
            source.write_bytes(content_bytes)
            return model_file_parts(
                source, mime_type, deadline=deadline, max_pages=max_pages
            )
    ...
    render_page_limit = (
        min(page_count, max(1, int(max_pages))) if max_pages is not None else page_count
    )
