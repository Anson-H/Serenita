...
from backend.app.domain.conversations.titles import (
    ...
    SESSION_TITLE_TARGET_CJK_CHARACTERS,
    SESSION_TITLE_TARGET_WORDS,
    SESSION_TITLE_FOCUS_KEYWORDS,
...
class ConversationTitleTasks:
    ...
    def start(self, account_id: str, turn, user_message: dict[str, Any]):
        ...
        should_generate = (
            ...
            and bool(user_message.get("generate_session_title"))
    ...
    def generate(
        ...
        *,
        ...
        try:
            request = self.prepare_request(messages, purpose="session_title_metadata")

            def check():
                cancellation_token.raise_if_cancelled()
                session = self.repository.session_row(
                    account_id, str(turn["session_id"])
                )
                if session is None or session["is_title_manual"]:
                    raise OperationCancelledError("会话已删除或标题已手工更新。")
            ...
            def invoke():
                ...
                return self.model_catalog.complete_chat_for_account(
            ...
            result = run_model_request(
                invoke,
                check_running=check,
                cancellation_token=cancellation_token,
                operation="session_title_metadata",
            )
        except OperationCancelledError:
            raise
        except Exception:
