...
from backend.app.application.notifications.registry import build_notification_types
...
from backend.app.application.notifications.types import (
    AccountNotificationScope,
)
from backend.app.repositories.notification_repository import NotificationRepository
from backend.app.repositories.conversations.notification_outbox import (
    ConversationNotificationOutboxRepository,
)
...
class NotificationService:
    def __init__(self, members, *, types=None):
        ...
        self.outbox = ConversationNotificationOutboxRepository(self.paths)
    ...
    def set_preferences(self, actor, notifications_enabled=_UNSET, **changes):
        ...
        if not changes or any(
            key not in allowed or type(value) is not bool
    ...
    def revalidate(self, actor, *, member_id=None):
        after = ""
        while True:
            rows = self.repository.active_resources(
                actor, member_id=member_id, after=after
            )
            if not rows:
                return
    ...
    def resources_changed(self, *, member_id, account_ids=None):
        try:
            accounts = (
                ...
                else self.repository.member_accounts(member_id)
            )
        except Exception:
            log.exception(
                "Failed to resolve notification recipients after resource commit"
            )
    ...
    def deliver(self, actor, event_id, content):
        ...
        with self.guard(actor, content["member_id"]) as access:
            ...
            if (
                ...
                or (
                    adapter.delivery_allowed is not None
            ...
            admission = (
                (lambda db, value: adapter.delivery_admission(db, value, resource))
                if adapter.delivery_admission
                else None
            )
            outcome = self.repository.deliver(
                actor, event_id, content, admission=admission
            )
    ...
    def dispatch(self, owner, at=None):
        ...
        for row in self.outbox.pending(owner, at):
            try:
                result = (
                ...
                self.outbox.finish_delivery(
                    owner,
                    row,
                    "pending" if result == "waiting" else result,
                    "NOTIFICATION_DEFERRED" if result == "waiting" else None,
                )
            except (SerenitaError, LookupError) as exc:
                if isinstance(exc, LookupError) or exc.kind in ("forbidden", "missing"):
                    self.outbox.finish_delivery(owner, row, "skipped")
                else:
                    self.outbox.finish_delivery(
