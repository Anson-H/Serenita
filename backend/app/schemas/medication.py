...
class Plan(StrictModel):
    ...
    starts_at: str = Field(description='保存用药计划时必填的用药计划开始时间，须有明确日期，采用本地 ISO 日期时间；日期精度取本地日边界，不接受未知或空值。')
    ends_at: str | None = Field(default=None, description='结束边界：具体日期采用本地 ISO 日期时间，边界不含该时刻；long_term 表示已明确长期使用，null 表示结束日期未知，省略默认为 null。')
    ...
    @model_validator(mode='after')
    def valid(self):
        self.timezone = local_timezone_name()
        self.starts_at = local_iso(self.starts_at)
        if self.ends_at not in (None, "long_term"):
            self.ends_at = local_iso(self.ends_at)
