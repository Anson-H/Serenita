...
import type {MedicationSource,MedicationItem} from "../../api/medications/medicationTypes";
import {addMedicationSources,medicationSourceUrl,readMedicationSource} from "../../api/medications/medicationApi";
...
export function MedicationSources({member,item,canEdit,onReload}:{member:string;item:MedicationItem|null;canEdit:boolean;onReload:()=>void}) {
  ...
  return <>
    <section className="report-overview-thumbnail medication-overview-files" aria-label="药品图片与文件">
      ...
      {canEdit?<><button className="control control--secondary report-source-add-button" type="button" disabled={!item||busy||!!uploads} onClick={()=>input.current?.click()}><UploadIcon/><span>{busy?'正在补充':'补充原件'}</span></button><input ref={input} hidden type="file" multiple accept="image/jpeg,image/png,image/heic,application/pdf,text/plain" onChange={e=>{const selectedFiles=Array.from(e.target.files??[]);e.currentTarget.value='';if(selectedFiles.length)void sendFiles({files:selectedFiles,request:randomUuid()});}}/></>:null}
