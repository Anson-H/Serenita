...
import { saveMedication, deleteMedication, saveMedicationCatalog, deleteMedicationCatalog } from "../../api/medications/medicationApi";
import type { MedicationIdentity, MedicationItem, MedicationKind, MedicationPlanInput, MedicationValues } from "../../api/medications/medicationTypes";
  ...
  const editor = useAutosaveResource<MedicationEditorDraft>({
    ...
    save: async draft => {
      ...
      const saved = draft.kind === "medication"
        ? await saveMedicationCatalog( changedMedicationValues(draft.values, previous?.kind === "medication" ? previous.values : null), id, operationId.current)
        : await saveMedication(member, "plan", changedMedicationValues(draft.values, previous?.kind === "plan" ? previous.values : null), id, operationId.current);
