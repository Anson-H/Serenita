...
import type {MedicationIdentity} from "../../api/medications/medicationTypes";
import { navigationLabels } from "../../components/navigationLabels";
import {useId,useRef,useState} from 'react';
import type {MedicationBatch} from "../../api/medications/medicationTypes";
import {saveBatch} from "../../api/medications/medicationApi";
...
export function MedicationBatchCreateDialog({member,medication,onClose,onSaved}:{member:string;medication?:string;onClose:()=>void;onSaved:(batch:MedicationBatch)=>void}) {
  ...
  const request=useRef({id:randomUuid(),payload:null as {quantity:string;expires_on:string|null;notes:string|null}|null,savedId:undefined as string|undefined}),running=useRef(false),formId=useId();
