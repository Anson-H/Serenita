...
import type {Medication,MedicationItem} from "../../api/medications/medicationTypes";
import {listMedicationCatalog,readMedicationCatalog} from "../../api/medications/medicationApi";
...
export function MedicationCatalog({detailOpen,onOpenDetail,onCloseDetail,onDetailTitleChange}:{detailOpen:boolean;onOpenDetail:()=>void;onCloseDetail:()=>void;onDetailTitleChange:(title:string)=>void}) {
  ...
  useEffect(()=>{onDetailTitleChange(selected?medicationName(selected):'药品目录');},[selected,onDetailTitleChange]);
