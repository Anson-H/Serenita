...
import { saveMemberPreferences, updateMember, type Member, type MemberFields } from "../../api/accounts/memberApi";
...
export function MemberInformationPanel({ member, onClose, panelRef }: {
  ...
  const relationshipLabel = member.is_owned ? "与本人的关系" : "与档案所有者的关系";
  ...
  const initialDraft: MemberFields = {
    member_name: member.member_name, relationship: member.relationship, sex: member.sex, birth_date: member.birth_date,
    ...
    <form className="member-information-pane" onSubmit={submit}>
      <div className="member-information-scroll scroll-content content-column">
        <GroupedList layout="fields" density="standard">
          {member.can_edit ? <MemberFieldsEditor relationshipLabel={relationshipLabel} value={draft} disabled={busy} onChange={changeDraft} /> : <>
            <ReadonlyField label="成员名称" value={member.member_name} />
            <ReadonlyField label={relationshipLabel} value={member.relationship ?? "未设置"} />
