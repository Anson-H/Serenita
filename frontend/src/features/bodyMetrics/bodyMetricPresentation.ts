...
export function newRecord(
  ...
  return {
    ...
    starts_at: localIsoString(new Date(`${date}T08:00:00`)),
    ends_at:
      kind === "sleep"
        ? localIsoString(new Date(`${date}T09:00:00`))
        : kind === "workout"
          ? localIsoString(new Date(`${date}T08:30:00`))
          : null,
...
export function range(date: string, period: string, customEnd: string) {
  ...
  return {
    after: dateOnly(a),
    before: period === "custom" ? customEnd : dateOnly(b),
  };
}
