...
import type { AddLabReportItemInput, ReportDetail, ReportEditableField } from "../../../api/reports/reportTypes";
...
export class ReportMutationActions extends ReportStore<MutationState> {
  ...
  updateSelectedReportField = async (input: { field: ReportEditableField; value: string | null; item_id?: string }) => {
    ...
    try {
      const updated = await this.serialTasks.run(key, () => reportApi.updateReportField(this.detail.memberId, key, input), operation.isCurrent);
  ...
  addSelectedReportLabItem = (input: AddLabReportItemInput) => this.mutateLabItem("adding",
    reportId => reportApi.addReportLabItem(this.detail.memberId, reportId, input), "检验指标已添加。", "检验指标添加失败。");

  deleteSelectedReportLabItem = (itemId: string) => this.mutateLabItem(itemId,
    reportId => reportApi.deleteReportLabItem(this.detail.memberId, reportId, itemId), "检验指标已删除。", "检验指标删除失败。");
  ...
  deleteSelectedReportAnalysis = async () => {
    ...
    this.options.onMessage("");
    try {
      const updated = await reportApi.updateReportField(this.detail.memberId, operation.reportId, { field: "analysis_content", value: null });
      if (!await this.apply(updated, operation)) return false;
  ...
  addSelectedReportSources = async (fileList: FileList | File[]) => {
    ...
    this.options.onMessage("");
    try {
      const updated = await reportApi.addReportSourceFiles(this.detail.memberId, operation.reportId, files);
      if (!await this.apply(updated, operation)) return false;
