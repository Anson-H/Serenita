...
import type { ReportDetail } from "../../../api/reports/reportTypes";
...
export function createReportFavoriteActions({
  ...
  async function favoriteReports(reportIds: string[]) {
    ...
    try {
      const outcomes = await Promise.all(pendingReportIds.map(async (reportId) => {
        try {
          const favorite = await favoriteApi.createFavoriteSource({
            memberId,
            sourceType: "report",
            sourceId: reportId,
            tags: []
  ...
  async function toggleReportFavorite() {
    ...
    try {
      if (reportFavorite) {
        await favoriteApi.deleteFavorite(reportFavorite.favorite_id);
        if (!isCurrentScope()) return false;
        setFavorites(current => current.filter(item => item.favorite_id !== reportFavorite.favorite_id));
      } else {
        const favorite = await favoriteApi.createFavoriteSource({ memberId, sourceType: "report", sourceId: selectedReportId, tags: [] });
        if (!isCurrentScope()) return false;
