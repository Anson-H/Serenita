...
import type { CreateReportInput } from "../../../api/reports/reportTypes";
...
export class ReportCollectionActions extends ReportStore<{ deleting: boolean; creating: boolean }> {
  ...
  deleteReports = async (reportIds: string[]) => {
    ...
    try {
      const outcomes = await Promise.all(unique.map(async reportId => {
        try { await reportApi.deleteReport(this.detail.memberId, reportId); return { reportId, success: true as const }; }
  ...
  createManualReport = async (input: CreateReportInput) => {
    ...
    this.options.onMessage("");
    try {
      const created = await reportApi.createReport(this.detail.memberId, input);
