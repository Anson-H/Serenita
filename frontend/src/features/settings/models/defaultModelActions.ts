import { updateModelDefaults, fetchModelDefaults } from "../../../api/models/modelProviderApi";
...
export function createDefaultModelActions({ defaultSaveSequences, addedModels, setModelDefaults, serialTasks, isCurrentScope }: Dependencies) {
  async function updateDefaultModel(usage: DefaultModelUsage, nextModelId: string) {
    ...
    try {
      ...
      const defaultsResponse = await serialTasks.current.run("defaults", () => updateModelDefaults({ [usage]: nextModelId || null }), isCurrentScope);
      ...
      try {
        const response = await fetchModelDefaults();
