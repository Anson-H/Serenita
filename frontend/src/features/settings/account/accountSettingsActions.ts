...
export function createAccountSettingsActions({
  ...
  async function autoSaveAccountProfile(
    ...
    try {
      const result = await serialTasks.current.run("account", () => accountSettingsApi.updateAccount(trimmedAccount, trimmedName), isCurrentScope);
      if (isCurrentScope()) {
  ...
  async function changePassword(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await accountSettingsApi.changePassword(
