...
import * as modelProviderApi from "../../api/models/modelProviderApi";
...
export function useConversationModelControl(options: ConversationModelControlOptions) {
  ...
  const defaultModel = modelCatalog.defaults.chat && isModelAvailable(modelCatalog.defaults.chat) && isGenerationModel(modelCatalog.defaults.chat) ? modelCatalog.defaults.chat : undefined;
  ...
  async function chooseSessionModel(modelId: string) {
    ...
    try {
      const defaultsResponse = await modelProviderApi.updateModelDefaults({ chat: modelId });
