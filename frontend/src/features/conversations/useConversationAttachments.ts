...
import type { ConversationDetail, UploadedResource } from "../../api/conversations/conversationTypes";
...
export function useConversationAttachments(options: ConversationAttachmentsOptions) {
  ...
  async function refreshUploadedDraft(draft: DraftRecord, sessionId: string) {
    ...
    try { detail = await conversationApi.getConversation(sessionId); }
  ...
  async function uploadFiles(files: File[], initialSessionId: string | null = null, batch?: {
    ...
    const run = async () => {
      ...
      for (const file of files) {
        ...
        try {
          const result = await conversationApi.uploadContextResource(batch?.memberId ?? memberId, sessionId, file, selectedModelId, value => {
