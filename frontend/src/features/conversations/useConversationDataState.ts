...
import type { ConversationDetail, ConversationSummary } from "../../api/conversations/conversationTypes";
...
export function useConversationDataState() {
  ...
  const index = useMemo(() => createConversationIndex(conversationApi.fetchConversations, publishConversations, isCurrent, setHasMoreConversations), [auth]);
