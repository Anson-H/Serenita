...
import { beginConnectionTestRequest, connectionTestRequestIsCurrent, invalidateConnectionTestRequest } from "../probes/settingsConnectionRequests";
import { draftsMatch, type ProviderDraft } from "./providerDraft";
import { type ProviderConnectionTestState, type TestState } from "../../../utils/requestStatus";

type Dependencies = {
  api: Pick<ModelSettingsApi, "saveModelProvider" | "testModelProvider" | "revealModelProviderCredential">;
...
export function createProviderSettingsActions({
  ...
  async function revealModelCredential(providerId: string): Promise<boolean> {
    ...
    try {
      const result = await api.revealModelProviderCredential(providerId);
  ...
  async function autoSaveProviderDraft(providerId: string, draft: ProviderDraft) {
    ...
    if (provider.provider_kind === "managed") return draft;
    try {
      ...
      const result = await serialTasks.current.run(`provider:${providerId}`, () => api.saveModelProvider(
