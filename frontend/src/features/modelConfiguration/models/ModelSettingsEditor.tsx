import { ModelServiceStatus } from "./ModelServiceStatus";
...
import { modelSettingsDraft, modelSettingsPatch, modelSettingsPayload, modelSettingsSignature } from "./modelSettingsDraft";
...
import type { ModelSettingsPage } from "./modelSettingsTypes";
...
export function ModelSettingsEditor(props: EditorProps) {
  return <><ModelServiceStatus model={props.model} />{isGenerationModel(props.model)
    ? <GenerationSettingsEditor {...props} model={props.model} />
    : <EmbeddingSettingsEditor embedded={props.embedded} probeDisabled={props.probeDisabled} supplierFallbackName={props.supplierFallbackName} page={props.page} onNavigate={props.onNavigate} providerAttachmentMimeTypes={props.providerAttachmentMimeTypes} model={props.model} onSave={props.onSave} onProbe={props.onProbe} onDelete={props.onDelete} probing={props.probing} saving={props.saving} />}</>;
}
...
function GenerationSettingsEditor({
  ...
  useEffect(() => {
    const savedDraft = modelSettingsDraft(model);
    const nextDraft = { ...savedDraft };
    for (const key of Object.keys(nextDraft) as (keyof ModelSettingsDraft)[]) {
      if (JSON.stringify(draftRef.current[key]) !== JSON.stringify(savedDraftRef.current[key])) {
        Object.assign(nextDraft, { [key]: draftRef.current[key] });
      }
    }
    const previousPayload = modelSettingsPayload(savedDraftRef.current);
    const savedPayload = modelSettingsPayload(savedDraft);
    // Pending edits must use the refreshed capabilities as their save baseline.
    saveQueueRef.current = saveQueueRef.current.map(job => {
      const target = { ...savedPayload, ...modelSettingsPatch(job.target, previousPayload) };
      return { target, signature: modelSettingsSignature(target) };
    });
    savedDraftRef.current = savedDraft;
    lastSavedPayloadRef.current = savedPayload;
    lastSavedSignatureRef.current = modelSettingsSignature(savedPayload);
  ...
  return (
    ...
    >
      <div className={embedded ? "model-settings-panel-body" : "model-settings-panel-body scroll-content content-column"}>
        {page === "root" ? (
          <>
            ...
            <button
              ...
              disabled={deleting || (!probing && (probeDisabled || saving))}
