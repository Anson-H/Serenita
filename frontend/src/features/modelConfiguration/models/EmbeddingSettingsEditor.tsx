...
import type { AddedModel, ModelUpdatePayload, ModelType, EmbeddingCapabilities } from "../../../api/models/modelTypes";
import { GroupedList } from "../../../components/GroupedList";
import { SelectPopover } from "../../../components/SelectPopover";
import { modelTypeLabels } from "../modelEligibility";
...
export function EmbeddingSettingsEditor({ model, onSave, onProbe, onDelete, probing, saving, page, onNavigate, providerAttachmentMimeTypes, supplierFallbackName, embedded = false, probeDisabled = false }: {
  ...
  model: AddedModel; onSave: Save; onProbe: () => void | Promise<void>; onDelete?: () => Promise<boolean>; probing: boolean; saving: boolean; embedded?: boolean; probeDisabled?: boolean;
}) {
  const [error, setError] = useState("");
  ...
  return <div className="model-settings-panel-form" data-embedded={embedded || undefined} key={model.model_id}><div className={embedded ? "model-settings-panel-body" : "model-settings-panel-body scroll-content content-column"}>
    {page === "root" ? <>
      ...
      <button aria-busy={probing} className={`control control--secondary model-capability-probe-button${probing ? " probing" : ""}`} type="button" disabled={!probing && (probeDisabled || saving || !!error)} onClick={async () => { if (!probing) await pending.current; await onProbe(); }}><LightningIcon /><span>{probing ? "检测中" : "检测模型"}</span></button>
      {onDelete ? <button className="control control--secondary control--danger model-delete-action" type="button" disabled={saving} onClick={() => void onDelete()}>删除模型</button> : null}
    </> : null}
