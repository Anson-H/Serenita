import { ModelServiceStatus, modelDisplayName } from "./ModelServiceStatus";
...
export function AddedModelList({ providerId, models, probingModelIds, onAdd, onOpen, onDelete }: {
  ...
  const actions = useListDeleteActions({
    ...
    entries: models.map(model => ({ id: model.model_id, name: modelDisplayName(model) })),
    onDelete: async ids => {
      const failed: string[] = [];
      for (const id of ids) {
    ...
    <div className="provider-model-content">
      ...
      <div className="added-model-list-area">
        ...
        <GroupedList className="model-list added-model-list" density="standard" selectionMode={actions.selectionMode ? "multiple" : undefined}>
          ...
          {models.map(model => {
            const probing = probingModelIds.includes(model.model_id);
            return <div className="model-item" data-grouped-list-item key={model.model_id}>
              <button {...actions.rowProps(model.model_id)}
                aria-label={actions.selectionMode ? `${actions.selected(model.model_id) ? "取消选择" : "选择"}模型：${modelDisplayName(model)}` : `打开模型详情：${modelDisplayName(model)}`}
                ...
                <span className="model-row-name" title={modelDisplayName(model)}>{modelDisplayName(model)}</span>
                <span className="model-row-actions">
                  <ModelServiceStatus model={model} /><AddedModelCapabilityIcons model={model} />
                  <span className="control-row-description model-type-label">{modelTypeLabels[model.model_type]}</span>
                  {probing ? <span className="control-row-description model-probe-status">检测中</span> : null}
