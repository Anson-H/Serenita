...
export function RemoteModelPicker({
  ...
  return (<div className="content-dialog-backdrop dialog-viewport-backdrop">
    <section aria-labelledby="model-picker-title" aria-modal="true" className="model-picker-modal creation-dialog dialog-viewport-surface dialog-title-ellipsis" ref={modelPickerDialogRef} role="dialog" tabIndex={-1}>
      ...
      <div className="dialog-body model-picker-body">
        <input
          ...
          placeholder={catalogOnly ? "搜索模型" : "搜索模型 ID"}
          type="search"
          value={modelPickerQuery}
        />
        {modelPickerLoading ? <p className="status-message testing">加载中...</p> : null}
        {modelPickerError ? <div role="status"><p>{modelPickerError}{catalogOnly ? "" : " 可继续输入模型 ID 手工添加。"}</p><button type="button" onClick={() => void retryLoadRemoteModels()}>重试读取目录</button></div> : null}
              ...
              return (
                ...
                >
                  ...
                  {!collapsed ? <GroupedList className="root-disclosure-content" selectionMode="multiple" density="standard">{group.models.map((model) => {
                    ...
                    const displayName = catalogOnly ? model.model_name : model.remote_model_id;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="model-picker-row-copy">
                          <span title={model.remote_model_id}>{catalogOnly ? model.model_name : identity.model}</span>
                        </div>
                      </button>
                    );
                  })}</GroupedList> : null}
        ...
        {!catalogOnly && !modelPickerLoading && modelPickerQuery.trim() && !remoteModels.some(model => model.remote_model_id === modelPickerQuery.trim()) && <button className="control control--secondary" type="button"
