import { request, requestResponse, API_BASE_URL } from "../transport/request";
...
export const saveMedication = (member: string,kind: 'plan',values: MedicationChangesByKind['plan'],id?: string,operationId?: string) => request<MedicationOfKind<'plan'>>(medicationBase(member,kind,id),{method:id?'PATCH':'POST',headers:operationId?{'X-Serenita-Operation-ID':operationId}:{},body:JSON.stringify(values)});
export const deleteMedication = (member: string,kind: 'plan',id: string) => request(medicationBase(member,kind,id),{method:'DELETE'});
export const saveBatch = (member: string,medicationId: string,values: Record<string,unknown>,id?: string,operationId?: string) => request<MedicationBatch>(medicationBase(member,'medication',medicationId)+'/batches'+(id?'/'+encodeURIComponent(id):''),{method:id?'PATCH':'POST',headers:operationId?{'X-Serenita-Operation-ID':operationId}:{},body:JSON.stringify(values)});
...
export function addMedicationSources(medicationId: string, files: File[], operationId: string) {
  const body = new FormData();
  for (const file of files) body.append('files', file);
  return request<Medication>(catalogueBase(medicationId) + '/source-files', {
    method: 'POST', headers: {'X-Serenita-Operation-ID': operationId}, body,
  });
}
...
const catalogueBase = (id?: string) => '/knowledge/medication-catalog' + (id ? '/' + encodeURIComponent(id) : '');
...
export const saveMedicationCatalog = (values:MedicationChangesByKind['medication'],id?:string,operationId?:string) => request<Medication>(catalogueBase(id),{method:id?'PATCH':'POST',headers:operationId?{'X-Serenita-Operation-ID':operationId}:{},body:JSON.stringify(values)});
