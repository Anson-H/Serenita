...
import { assertAuthContext, captureAuthContext } from "../auth/authLifecycle";
import { publishAuthInvalidation } from "../auth/authSessionEvents";
...
export async function requestResponse(path: string, init?: RequestInit): Promise<Response> {
  ...
  if (context.accountId && !["GET", "HEAD", "OPTIONS"].includes((init?.method ?? "GET").toUpperCase())) {
    headers.set(path.startsWith("/admin/") ? "X-Serenita-Administrator-ID" : "X-Serenita-Account-ID", context.accountId);
    if (!headers.has("X-Serenita-Operation-ID")) {
      headers.set("X-Serenita-Operation-ID", randomUuid());
    }
  }
