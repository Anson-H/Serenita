...
export const previewImport = (m: string, file: File, operationId: string) => {
  const data = new FormData();
  data.append("file", file);
  return request<ImportJob>(
    `${bodyBase(m)}/imports/preview`,
    { method: "POST", headers: { "X-Serenita-Operation-ID": operationId }, body: data },
  );
};
