...
let AUTH_SESSION_EVENT = "serenita:auth-session-changed";
let AUTH_SESSION_STORAGE_KEY = "serenita:auth-session-change";
export function configureAuthSessionScope(scope: string) {
  AUTH_SESSION_EVENT = `serenita:${scope}:auth-session-changed`;
  AUTH_SESSION_STORAGE_KEY = `serenita:${scope}:auth-session-change`;
}
...
export function publishAuthInvalidation(status: number, code?: string | null) {
  if (status !== 401 || code === "SIGN_IN_FAILED" || code?.startsWith("OFFICIAL_")) return;
