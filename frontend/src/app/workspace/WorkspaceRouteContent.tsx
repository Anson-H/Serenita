...
import * as reportApi from "../../api/reports/reportApi";
...
const KnowledgeWorkspace = lazy(() => import("../../features/knowledge/KnowledgeWorkspace").then(({KnowledgeWorkspace}) => ({default:KnowledgeWorkspace})));
...
const MemoryWorkspace = lazy(() => import("../../features/memory/MemoryWorkspace").then(({MemoryWorkspace}) => ({default:MemoryWorkspace})));
...
export function WorkspaceRouteContent({
  ...
  function renderDeferredWorkspace(content: ReactNode, label: string) {
    return (
      <DeferredContentBoundary key={isSettingsRoute(route) ? "settings" : route}>
  ...
  function renderWorkspaceShell(mainContent: ReactNode) {
    return (
      <WorkspaceRouteShell
        memoryMemberId={memberIdFromHealthPath(route) ?? members.activeMemberId ?? members.collection.default_member_id ?? members.collection.members[0]?.member_id}
        healthNavigation={<HealthMemberNav members={members.collection.members}
          ...
          current={!isMemoryRoute(route) && (Boolean(memberIdFromHealthPath(route)) || isReportRoute(route))}
          onOpen={id => lifecycle.navigateTo(healthPathForMember(id))}
  ...
  async function openStandaloneReport(reportId: string, memberId: string | null = reportWorkspace.memberId || null) {
    ...
    if (memberId !== reportWorkspace.memberId) {
      try {
        await reportApi.getReport(memberId, reportId);
    ...
    return renderWorkspaceShell(
      renderDeferredWorkspace(
        <ReportWorkspacePanel
          ...
          informationRequested={new URLSearchParams(route.split('?')[1]).get('section') === 'information'}
  ...
  function renderRoute() {
    ...
    if (healthMemberId) {
      ...
      if (member && isMemoryRoute(route)) return renderWorkspaceShell(renderDeferredWorkspace(
        <MemoryWorkspace key={member.member_id} member={member} accessRevision={members.collection.access_revision} onNavigate={lifecycle.navigateTo}
          members={members.collection.members} onSelectMember={id=>members.selectMember(id,{type:"memory"})} sidebarToggle={workspaceShell.renderSidebarToggle("memory-sidebar-toggle")}
          />,
        "正在载入长期记忆…"));
    ...
    if (route === KNOWLEDGE_PATH) return renderWorkspaceShell(renderDeferredWorkspace(<KnowledgeWorkspace key={session.account_id} onReportsChanged={reportWorkspace.invalidateReportData} sidebarToggle={workspaceShell.renderSidebarToggle("knowledge-sidebar-toggle")} />, "正在载入知识库…"));
